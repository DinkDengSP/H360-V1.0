...
**Author: DengXiaoJun
**Date: 2020-06-19 15:37:18
**LastEditors: DengXiaoJun
**LastEditTime: 2020-11-04 10:58:04
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
//拷贝内存块
void UserMemCopy(void *des,const void *src,uint32_t n)
{
    uint8_t *xdes=des;
...
#pragma pack(4)
//内存池,可分配内存用一个大数组保存 
static uint8_t sramIS62_Base[MAX_SIZE_IS62]@".sram_is62";	
#pragma pack()				
//内存管理表,用于管理维护内存池的状态
static uint16_t sramIS62_MapBase[ALLOC_TABLE_SIZE_IS62]@".sram_is62";
//当前内存区域的内存管理是否就绪	
static uint8_t sramIS62_MemReady = 0;

//初始化内存块
void UserMemInitMemIS62(void)
{
    UserMemSet(sramIS62_Base,0,MAX_SIZE_IS62);
    UserMemSet(sramIS62_MapBase,0,ALLOC_TABLE_SIZE_IS62*2);
	//内存管理初始化OK 
	sramIS62_MemReady = 1;
}
//申请内存
void* UserMemMallocMemIS62(uint32_t size)
{
    int32_t offset = 0;
    //需要的内存块数
    uint16_t nmemb;
    ...
    //获取需要分配的连续内存块数
    nmemb = size / BLOCK_SIZE_IS62;
    if (size % BLOCK_SIZE_IS62)
    {
        //只能多不能少
        nmemb++;
    }
    //搜索整个内存控制区
    for (offset = ALLOC_TABLE_SIZE_IS62 - 1; offset >= 0; offset--)
    {
...
//释放内存
void UserMemFreeMemIS62(void* ptr)
{
    uint32_t offset;
    int i = 0;
    ...
    if(offset < MAX_SIZE_IS62)
    {
        //偏移所在内存块号码
        int index = offset/BLOCK_SIZE_IS62;	
		//内存块数量		
...
//内存块扩容
void* UserMemReallocMemIS62(void *ptr,uint32_t size)
{
    uint8_t* newPtr = UserMemMallocMemIS62(size);
    if(newPtr != NULL)
    {
        UserMemCopy(newPtr,ptr,size);
        UserMemFreeMemIS62(ptr);
        return (void*)newPtr;
    }
    else
    {
...
//获取空间使用率
uint8_t UserMemPerUsedMemIS62(void)
{
    uint32_t used = 0;
    uint32_t i = 0;
    for(i=0;i< ALLOC_TABLE_SIZE_IS62;i++)  
    {  
        if(sramIS62_MapBase[i])
			used++; 
    }
    return (used*100)/(ALLOC_TABLE_SIZE_IS62);
}
