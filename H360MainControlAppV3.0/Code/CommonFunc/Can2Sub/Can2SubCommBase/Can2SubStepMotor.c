...
**LastEditTime: 2020-01-16 15:40:59
**FilePath: \DeviceMainApplicationc:\LiHe\Local\02-Development\Software\H360_STM32\H360MainControlAppV2.2\Code\CommonFunc\Can2SubCommBase\Can2SubStepMotor.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
LH_ERR Can2SubSM_ReadPositionWhileReturn(CAN2_SUB_SM_INDEX motorIndex,int32_t* motorPos)
{
    ...
    if(can2ReadData.recvDataPtr == NULL)
    {
        return LH_ERR_BOARD_MAIN_COMM_SM_DATA_NULL;
    }
    //接收到的数据不需要
    if(can2ReadData.recvDatLength != 6)
    {
        //释放内存
        UserMemFree(MEM_AXI_SRAM,can2ReadData.recvDataPtr);
        return LH_ERR_BOARD_MAIN_COMM_SM_DATA_LENGTH;
    }
    //读取数据
    if(CAN2_SUB_DATA_MOTOR != Can2ProcUtilConvertArrayToUint16(can2ReadData.recvDataPtr,0))
    {
        //数据标志位错误
        UserMemFree(MEM_AXI_SRAM,can2ReadData.recvDataPtr);
        return LH_ERR_BOARD_MAIN_COMM_SM_DATA_FLAG;
    }
    //读取坐标
