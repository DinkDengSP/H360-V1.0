...
**LastEditTime: 2020-01-04 15:38:32
**FilePath: \DeviceMainApplicationc:\LiHe\Local\02-Development\Software\H360_STM32\H360MainControlAppV2.2\Code\CommonFunc\Can2SubCommBase\Can2SubInputRead.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
LH_ERR Can2SubPortInReadState(CAN2_SUB_INPUT* input)
{
    ...
    {
        input->inputPinIndex = pinIndexLocal;
        return LH_ERR_BOARD_MAIN_COMM_INPUT_PIN_DATA_NULL;
    }
    //接收到的数据不需要
    if(can2ReadData.recvDatLength != 4)
    {
        ...
        input->inputPinIndex = pinIndexLocal;
        //释放内存
        return LH_ERR_BOARD_MAIN_COMM_INPUT_PIN_DATA_LENGTH;
    }
    //读取数据
    if(CAN2_SUB_DATA_IO_IN != Can2ProcUtilConvertArrayToUint16(can2ReadData.recvDataPtr,0))
    {
        ...
        input->inputPinIndex = pinIndexLocal;
        //数据标志位错误
        return LH_ERR_BOARD_MAIN_COMM_INPUT_PIN_DATA_FLAG;
    }
    if(can2ReadData.recvDataPtr[2] != input->inputPinIndex)
    {
        ...
        input->inputPinIndex = pinIndexLocal;
        //数据标志位错误
        return LH_ERR_BOARD_MAIN_COMM_INPUT_PIN_DATA_INDEX;
    }
    //读取坐标
