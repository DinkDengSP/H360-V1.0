...
**LastEditTime: 2020-07-03 16:14:15
**FilePath: \Applicationd:\SVN_Cloud\H200\EmbeddedSoftware\H360\H360MainControlAppV2.2\Code\CommonFunc\Can2Protocol\Can2ProcMain.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
//计算指定信道使用的互斥信号量的数量
int Can2ProcCalcMutexCount(CAN2_SUB_BOARD boardID)
{
    uint8_t indexChannelUtil = 0;
    uint8_t indexBoardID = (uint8_t)(boardID-1);
    uint8_t mutexUsedCount = 0;
    for(indexChannelUtil = 0; indexChannelUtil < SYSTEM_CAN2_BOARD_CHANNEL_MAX; indexChannelUtil++)
    {
        if(0 != (can2RecvCache.boardCacheArray[indexBoardID].channelCacheArray[indexChannelUtil].mutexCan2Receive.OwnerNestingCtr))
        {
            //互斥信号量已经被申请
            mutexUsedCount++;
        }
    }
    return mutexUsedCount;
}
...
//CAN2接收处理一个数据单元
void Can2RecvProc(MCU_CAN_RECV_DAT* rxMsg)
{
    //ID解析
    Can2RecvAnalsisID(rxMsg->id,&can2ID_AnalysisResult);
    //然后对数据进行拷贝
    currentCan2RecvDataLength = rxMsg->dataLength;
    for(uint8_t i = 0; i < currentCan2RecvDataLength; i++)
    {
        currentCan2RecvDataBuffer[i] = rxMsg->dataRecvBuffer[i];
    }
    //查看板号和通道号是否支持
...
static CAN2_RECV_CHANNEL_UNIT* Can2ProcUtilGetChannelCacheUnitPtr(uint8_t targetBoardAddr,uint8_t channelNo,LH_ERR* errCode)
{
    ...
    {
        *errCode = LH_ERR_BOARD_MAIN_COMM_CAN2_BOARD;
        return NULL;
    }
    if(channelNo >= SYSTEM_CAN2_BOARD_CHANNEL_MAX)
    {
        *errCode = LH_ERR_BOARD_MAIN_COMM_CAN2_CHANNEL;
        return NULL;
    }
    //找到通道缓存
    ...
    {
        *errCode = LH_ERR_BOARD_MAIN_COMM_CAN2_MUTEX_OWNER;
        return channelUniPtr;
    }
    else
    {
        //获取不到互斥信号量
        *errCode = LH_ERR_BOARD_MAIN_COMM_CAN2_MUTEX_BUSY;
        return NULL;
    }
}
...
static LH_ERR Can2CheckCommandStruct(CAN2_SEND_CMD* sendCommand)
{
    if(sendCommand == NULL)
    {
        return LH_ERR_BOARD_MAIN_COMM_CAN2_CMD_NULL;
    }
    if(sendCommand->timeoutMs == 0)
    {
        return LH_ERR_BOARD_MAIN_COMM_CAN2_CMD_TIMEOUT_ZERO;
    }
    if (sendCommand->paramLength >= SYSTEM_CAN2_SINGLE_RECV_DATA_MAX)
    {
        return LH_ERR_BOARD_MAIN_COMM_CAN2_CMD_LENGTH_MAX;
    }
    return LH_ERR_NONE;
}
...
static void Can2ResetCommandChannel(CAN2_RECV_CHANNEL_UNIT* channelUnitPtr)
{
    ...
    {
        UserMemFree(MEM_AXI_SRAM,channelUnitPtr->dataRecvUnitPtr);
    }
    //然后再申请内存
    channelUnitPtr->dataRecvUnitPtr = NULL;
    do
    {
        channelUnitPtr->dataRecvUnitPtr = UserMemMalloc(MEM_AXI_SRAM,sizeof(CAN2_RECV_DATA_UNIT));
        if(channelUnitPtr->dataRecvUnitPtr == NULL)
        {
            CoreDelayMinTick();
        }
...
static void Can2SendCommandWithShortPack(CAN2_SEND_CMD* sendCommand,uint8_t frameCode)
{
    ...
    //拷贝数据
    txMsg.dataLength = 4 + sendCommand->paramLength;
    //帧ID
    txMsg.id = sendCommandId;
    //扩展帧
    txMsg.idMode = MCU_CAN_ID_MODE_EXT;
    //首先将指令转换为4字节数组
    Can2ProcUtilConvertUint32ToArray(txMsg.dataSendBuffer,0,sendCommand->commandCode);
    //然后将参数拷贝
    UserMemCopy(txMsg.dataSendBuffer+4,sendCommand->paramBuffer,sendCommand->paramLength);
    //发送数据
    MCU_CAN2_SendMsg(&txMsg);
}
...
static void Can2SendCommandWithLongPackStart(CAN2_SEND_CMD* sendCommand,uint8_t frameCode)
{
    ...
    txMsg.idMode = MCU_CAN_ID_MODE_EXT;
    //将数据长度转换到数组
    Can2ProcUtilConvertUint16ToArray(txMsg.dataSendBuffer,0,sendCommand->paramLength + 4);
    //将包数转换到数组
    uint8_t packCount = 0;
    //计算包数
    ...
    //包数写入
    txMsg.dataSendBuffer[2] = packCount;
    //发送数据
    MCU_CAN2_SendMsg(&txMsg);
}
...
static void Can2SendCommandWithLongPackData(CAN2_SEND_CMD* sendCommand,uint8_t frameCode)
{
    ...
    for(currentPackIndex = 0; currentPackIndex < packCount; currentPackIndex++)
    {
        if(currentPackIndex == 0)
        {
            //第一包,要带指令
            txMsg.dataLength = 8;
            txMsg.dataSendBuffer[0] = currentPackIndex;
            //首先将指令转换为4字节数组
            Can2ProcUtilConvertUint32ToArray(txMsg.dataSendBuffer,1,sendCommand->commandCode);
            //然后将参数拷贝
            UserMemCopy(txMsg.dataSendBuffer+5,sendCommand->paramBuffer,3);
            //发送数据
            MCU_CAN2_SendMsg(&txMsg);
        }
        else
        {
            //不是第一包
            txMsg.dataSendBuffer[0] = currentPackIndex;
            txMsg.dataLength = ((currentPackIndex != (packCount-1))?8:(1+((sendCommand->paramLength + 4)%7)));
            if(txMsg.dataLength == 1)txMsg.dataLength = 8;//如果长包长度为1,说明正好有一整包数据要发
            //数据拷贝
            UserMemCopy(txMsg.dataSendBuffer+1,sendCommand->paramBuffer + (currentPackIndex*7) - 4,txMsg.dataLength -1);
            //发送数据
            MCU_CAN2_SendMsg(&txMsg);
        }
    } 
...
LH_ERR Can2SendCommandWhileAck(CAN2_SEND_CMD* sendCommand)
{
    ...
    if(errorCode == LH_ERR_BOARD_MAIN_COMM_CAN2_MUTEX_OWNER)
    {
    ...
    if(channelUnitPtr->commandSendRetryCount > SYSTEM_CAN2_ACK_RETRY_MAX)
    {
        ...
        return LH_ERR_BOARD_MAIN_COMM_CAN2_ACK_RETRY_OUT;
    }
    if(channelUnitPtr->channelState == CAN2_RECV_CMD_STATE_ERR_ACK)
    {
        ...
        //释放互斥信号量
        OSMutexPost(&(channelUnitPtr->mutexCan2Receive),OS_OPT_POST_FIFO,&osErr);
        SystemPrintf("E>AckErr1:%d,%d,%08X,%d,%d,%d\r\n",sendCommand->targetBoardID,sendCommand->channelNo,sendCommand->commandCode,
                                                                channelUnitPtr->ackCode,channelUnitPtr->channelState,channelUnitPtr->commandSendRetryCount);
        return LH_ERR_BOARD_MAIN_COMM_CAN2_ACK_ERR;
    }
                                        ...
                                        CAN2_READ_DATA* readData)
{
    ...
    else
    {
        ...
        return LH_ERR_BOARD_MAIN_COMM_CAN2_WAIT_RETURN_LOGIC_ERR;
    }
    ...
    if(OSTimeGet(&osErr) - channelUnitPtr->commandSendOutStartMs > timeOutMs)
    {
        ...
        OSMutexPost(&(channelUnitPtr->mutexCan2Receive),OS_OPT_POST_FIFO,&osErr);
        //返回超时
        return LH_ERR_BOARD_MAIN_COMM_CAN2_RETURN_TIME_OUT;
    }
    if(channelUnitPtr->channelState != CAN2_RECV_CMD_STATE_COMPLETE)
    {
        ...
        OSMutexPost(&(channelUnitPtr->mutexCan2Receive),OS_OPT_POST_FIFO,&osErr);
        //返回超时
        return LH_ERR_BOARD_MAIN_COMM_CAN2_WAIT_RETURN_UNKNOW;
    }
    //指令执行完成,先拷贝数据包
    if(channelUnitPtr->dataRecvUnitPtr->currentDataState == CAN2_RECV_DATA_STATE_END)
    {
        ...
        do
        {
            readData->recvDataPtr = UserMemMalloc(MEM_AXI_SRAM,readData->recvDatLength);
...
LH_ERR Can2SendCommandWhileReturn(CAN2_SEND_CMD* sendCommand,CAN2_READ_DATA* readData)
{
    ...
    if(errorCode == LH_ERR_BOARD_MAIN_COMM_CAN2_MUTEX_OWNER)
    {
        return LH_ERR_BOARD_MAIN_COMM_CAN2_MUTEX_LAST_ACK_WAIT;
    }
    //复位通道缓存
    Can2ResetCommandChannel(channelUnitPtr);
    do
    ...
    if(channelUnitPtr->channelState == CAN2_RECV_CMD_STATE_ERR_ACK)
    {
        ...
        //释放互斥信号量
        OSMutexPost(&(channelUnitPtr->mutexCan2Receive),OS_OPT_POST_FIFO,&osErr);
        SystemPrintf("E>AckErr2:%d,%d,%08X,%d,%d,%d\r\n",sendCommand->targetBoardID,sendCommand->channelNo,sendCommand->commandCode,
                                                                channelUnitPtr->ackCode,channelUnitPtr->channelState,channelUnitPtr->commandSendRetryCount);
        return LH_ERR_BOARD_MAIN_COMM_CAN2_ACK_ERR;
    }
