...
//用于循环队列存储数据的数组
static uint8_t bufferWithUart1Send[LENGTH_UART1_BUFFER_WITH_SEND_VERSION0]@".axi_sram" = {0};
//用于printf的缓冲区
...
static void MCU_Uart1BaseInit(uint32_t baud, MCU_UART_LENGTH length, MCU_UART_STOPBIT stopBit,MCU_UART_CHECK_MODE checkMode, MCU_UART_HARD_CONTROL hardWareControl, MCU_UartRecvIntProcFunc rxCallBack)
{
    ...
    //USART2，USART1，UART4，UART5，UART6，UART7和UART8选择的时钟是PLCK1，时钟100MHz。
        ...
        RCC_PeriphClkInit.Usart234578ClockSelection = RCC_USART1CLKSOURCE_D2PCLK2;
...
static void MCU_Uart1InitData(void)
{
    OS_ERR err;
    //数据结构初始化
        ADT_CircleByteQueueInit(&circleByteQueueUart1Send, bufferWithUart1Send, LENGTH_UART1_BUFFER_WITH_SEND_VERSION0);
    //创建一个互斥信号量,用于串口发送
...
//串口初始化
void MCU_Uart1InitVersion0(uint32_t baud, MCU_UART_LENGTH length, MCU_UART_STOPBIT stopBit,MCU_UART_CHECK_MODE checkMode, MCU_UART_HARD_CONTROL hardWareControl, 
...
void MCU_Uart1SendBufferVersion0(uint8_t* bufferStartPtr,uint16_t sendLength)
{
    ...
    if(usart1SendCount >= USART1_SEND_FORCE_RESET_LIMIT_VERSION0)
    {
        usart1SendCount = 0;
        /* 等待中断空闲 */
        while(flagUart1Trans == 1)
        {
            ...
            //如果等待时间超阈值,退出等待
            if(usart1SendCountDelayMS > USART1_SEND_FORCE_RESET_DELAY_MS_VERSION0)
                break;
        }
        /* 重新复位USART */
        MCU_Uart1BaseDeInit();
...
void MCU_Uart1SendStringVersion0(uint8_t* stringStartPtr)
{
    ...
    MCU_Uart1SendBufferVersion0(stringStartPtr, (uint16_t)stringLength);
}
...
int MCU_Uart1PrintfVersion0(const char *format, ...)
{
    ...
    if ((rv > 0) && (rv <= LENGTH_UART1_BUFFER_WITH_FORMAT))
    {
        ...
        MCU_Uart1SendBufferVersion0(blockBufferWithUart1Printf, (uint16_t)rv);
    }
    //释放互斥信号量,禁止在释放信号量的时候出发任务调度
    if (OSRunning)
    {
