...
**LastEditTime: 2020-07-02 20:25:29
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
uint32_t TaskImplBoardIapCalcCRC32Table(const uint8_t *pu8Data, uint32_t u32Size)
{
    uint32_t i, crc;
    crc = 0xFFFFFFFF;
...
//APP的最大长度
#define MAX_LENGTH_APP_BYTES        ((1024-128)*1024 + 4)

//升级记录
static BOARD_IAP_RECORD boardIapRecord;
//缓存数据
static uint8_t boardIapArray[MAX_LENGTH_APP_BYTES]@".sram_is62";
...
BOARD_IAP_STATE TaskImplBoardIapCalcCRC32(uint8_t boardID,uint32_t crc32Result)
{
    ...
    //计算校验和
    uint32_t crc32Loacl = TaskImplBoardIapCalcCRC32Table(boardIapRecord.recvDataBuffer,boardIapRecord.exceptRecvDataLengthBytes);
