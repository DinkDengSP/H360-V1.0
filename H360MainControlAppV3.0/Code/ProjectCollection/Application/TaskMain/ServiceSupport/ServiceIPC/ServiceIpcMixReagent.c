...
**Author: DengXiaoJun
**Date: 2019-12-31 14:19:43
**LastEditors: DengXiaoJun
**LastEditTime: 2020-01-09 16:38:15
**FilePath: \DeviceMainApplication\TaskMain\ServiceSupport\ServiceIPC\ServiceIpcMixReagent.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
**ModifyRecord5:    
******************************************************************/
#include "ServiceIpcMixReagent.h"
#include "ServiceTaskConfig.h"

//IPC调用的数据结构
static IPC_DATA_MIX_REAGENT dataIPC_MixReagent;
//互斥信号量,用于API的互斥访问
...
{
    IPC_CMD_MIX_REAGENT_RESET_ALL         = 0X0000,//试剂混匀复位
    IPC_CMD_MIX_REAGENT_ROTATE_INTEGRATE  = 0X0001,//试剂混匀集成混匀
}IPC_CMD_MIX_REAGENT;
...
void IPC_ServiceMixReagentDataInit(void)
{
    ...
    dataIPC_MixReagent.can1ReportLevel = CAN1_REPORT_ERR_LEVEL_NORMAL;
    dataIPC_MixReagent.can1ReportCode = CAN1_REPORT_ERR_OK;
    //数据初始化
...
STATE_IPC IPC_ServiceMixReagentGetLastCommandStateAndResult(LH_ERR* resultCode,CAN1_REPORT_ERR_LEVEL* can1ReportErrorLevel,CAN1_REPORT_ERR_CODE* can1ReportErrorCode)
{
    ...
    *can1ReportErrorLevel = dataIPC_MixReagent.can1ReportLevel;
    *can1ReportErrorCode = dataIPC_MixReagent.can1ReportCode;
    //释放信号量
...
LH_ERR IPC_ServiceMixReagentWaitLastReturn(CAN1_REPORT_ERR_LEVEL* can1ReportErrorLevel,CAN1_REPORT_ERR_CODE* can1ReportErrorCode)
{
    ...
    do
    {
        ipcState = IPC_ServiceMixReagentGetLastCommandStateAndResult(&errorCode,can1ReportErrorLevel,can1ReportErrorCode);
...
//动作API发送任务给动作API
static LH_ERR IPC_ServiceMixReagentSendCommandWhileAck(uint16_t cmdIndex,int32_t param1,int32_t param2,int32_t param3,int32_t param4,int32_t param5)
{
    OS_ERR err;
    //系统消息指针
    ...
    //将申请的指针传递给对应的任务信道做处理
    OSTaskQPost((OS_TCB *)&tcbTaskServiceMixReagent, //向协议处理线程发送消息
                        (void *)systemTaskCmd,
...
{
    return IPC_ServiceMixReagentSendCommandWhileAck(IPC_CMD_MIX_REAGENT_RESET_ALL,0,0,0,0,0);
}

//试剂混匀集成混匀,不等待指令完成,非堵塞
LH_ERR IPC_ServiceMixReagentRotateIntegrateWhileAck(MIX_MODE mode)
{
    return IPC_ServiceMixReagentSendCommandWhileAck(IPC_CMD_MIX_REAGENT_ROTATE_INTEGRATE,(int32_t)(mode),0,0,0,0);
}
...
LH_ERR IPC_ServiceMixReagentResetAllWhileReturn(CAN1_REPORT_ERR_LEVEL* can1ReportErrorLevel,CAN1_REPORT_ERR_CODE* can1ReportErrorCode)
{
    ...
    //首先发出指令,不等待返回看看是否能够发送出去
        errorCode = IPC_ServiceMixReagentSendCommandWhileAck(IPC_CMD_MIX_REAGENT_RESET_ALL,0,0,0,0,0);
    ...
    //等待指令完成
        errorCode = IPC_ServiceMixReagentWaitLastReturn(can1ReportErrorLevel,can1ReportErrorCode);
        if(errorCode != LH_ERR_NONE)
        {
            return errorCode;
        }
...
LH_ERR IPC_ServiceMixReagentRotateIntegrateWhileReturn(MIX_MODE mode,CAN1_REPORT_ERR_LEVEL* can1ReportErrorLevel,CAN1_REPORT_ERR_CODE* can1ReportErrorCode)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //首先发出指令,不等待返回看看是否能够发送出去
        errorCode = IPC_ServiceMixReagentSendCommandWhileAck(IPC_CMD_MIX_REAGENT_ROTATE_INTEGRATE,(int32_t)(mode),0,0,0,0);
        if(errorCode != LH_ERR_NONE)
        {
            return errorCode;
        }
