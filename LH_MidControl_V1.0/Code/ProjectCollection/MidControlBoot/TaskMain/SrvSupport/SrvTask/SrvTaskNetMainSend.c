...
**LastEditTime: 2022-01-12 17:32:49
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
void TaskFuncSrvNetMainSend(unsigned long threadInput)
{
    ...
    while(1)
    {
        ...
        if((txResult == TX_SUCCESS)&&(netSendMsgPtr != NULL))
        {
            ...
            {
                //数据结构体为空,报警并且结束本次循环
                SystemPrintf("[MID]Boot W>TcpNet SendDataMsgBuffer NULL!!!\r\n");
            }
            //获取到有效的数据需要发送
            if (SOCK_ESTABLISHED == TcpBaseUtilGetMainPortState())
            {
                ...
                for(tcpDataSendRepeatCount = 0; tcpDataSendRepeatCount < TCP_DATA_SEND_RETRY_MAX;tcpDataSendRepeatCount++)
                {
                    ...
                    if(LH_ERR_NONE != BoardW5500_SendBufferMain( netSendMsgPtr->dataBufferPtr, netSendMsgPtr->dataLength))
                    {
                        ...
                        SystemPrintf("[MID]Boot W>NetChipDataSendFail,RetryMax:3,W5500 Error,Cmd: 0X%04X\r\n",netSendMsgPtr->command);
                    ...
                    {
                        //超时没获取到ACK
                        SystemPrintf("[MID]Boot W>PendTcpAckSemTimeOut,Error : %d,Cmd : %d\r\n", txResult,netSendMsgPtr->command);
                        //重发次数未超过三次,进行下一次重发
                    ...
                    SystemPrintf("[MID]Boot W>TcpRecvAckFlag : %d\r\n", TcpBaseUtilGetNetAckFlag());
                    //下一次循环重发
                ...
                {
                    //打印数据为什么发送失败
                    SystemPrintf("[MID]Boot E>Tcp Send DataErrorOccor! Reason : 0X%02X\r\n", lastDataSendErrState);
                }
                //可选的降低通讯速率函数       
            ...
            else
            {
                ...
                SystemPrintf("[MID]Boot W>TcpSendDataFail,NetNotConnect,TcpState : %02X, TcpCmd: %04X, TcpDataLength : %d\r\n", 
                                TcpBaseUtilGetMainPortState(), //网口状态
                                netSendMsgPtr->command,//指令码
                                netSendMsgPtr->dataLength//发送数据长度
                            );
                //等待网络连接
                while(SOCK_ESTABLISHED != TcpBaseUtilGetMainPortState())
                {
                    CoreDelayMs(10);
                }
            }
            ...
            if(netSendMsgPtr->dataBufferPtr != NULL)
            {
                UserMemFree(MEM_AXI_SRAM,netSendMsgPtr->dataBufferPtr);
            }
            //释放消息内存
            UserMemFree(MEM_AXI_SRAM,netSendMsgPtr);
            //设置消息指针为空
            netSendMsgPtr = NULL;
        }
    }
