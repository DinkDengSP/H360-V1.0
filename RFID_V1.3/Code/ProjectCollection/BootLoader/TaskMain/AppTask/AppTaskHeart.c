...
void TaskFuncHeart(void *p_arg)
{
	...
	OS_MSG_SIZE size;
    //设置系统状态
    SytemModeInit(SYSTEM_MODE_TYPE_BOOT);
    errorCode = ServiceImplDebugIapBoardIapInit();
    if(errorCode != LH_ERR_NONE)
    {
        SystemPrintf("TaskFuncHeart ServiceImplDebugIapBoardIapInit error:%08x\r\n",errorCode);
        return;
    }
    //任务接收讯息
...
void ServiceTaskFuncHeart_UartMsg_Proc_UpdateStart(UART_CMD_UTIL* uartCmdUtilPtr,uint8_t* detailDebugMsgPtr)
{
    ...
    LH_ERR errorCode = LH_ERR_NONE;
    if(uartCmdUtilPtr->cmdDatLen != 4)
    {
        errorCode = LH_ERR_IAP_UPDATE_START_DATA_LENGTH;
    }
    else
    {
        packCount = 0;
        //获取升级数据字节数
        paramUtil = ConvertArrayToUint32HighFirst((uartCmdUtilPtr->cmdDat),0);
    }
    //任务开始
    AppShowStartMsg(AppHeartDebugStartMsg,detailDebugMsgPtr,paramUtil,0,0,0,0);
    ServiceImplDebugIapStart(paramUtil);
    //返回数据
...
void ServiceTaskFuncHeart_UartMsg_Proc_UpdateData(UART_CMD_UTIL* uartCmdUtilPtr,uint8_t* detailDebugMsgPtr)
{
    ...
    packCount++;
    //任务开始
    AppShowStartMsg(AppHeartDebugStartMsg,detailDebugMsgPtr,uartCmdUtilPtr->cmdDatLen,packCount,uartCmdUtilPtr->cmdDatLen,0,0);
    //实际调用
    errorCode = ServiceImplDebugIapTransData(uartCmdUtilPtr->cmdDatLen,uartCmdUtilPtr->cmdDat);
    //返回数据
    Uart1CmdSendAckPack(uartCmdUtilPtr->funcCode,errorCode,0,NULL);
    //任务结束
    AppShowEndMsg(AppHeartDebugEndMsg,LH_ERR_NONE);
...
void ServiceTaskFuncHeart_UartMsg_Proc_UpdateEnd(UART_CMD_UTIL* uartCmdUtilPtr,uint8_t* detailDebugMsgPtr)
{
    ...
    uint32_t crcDat = 0;
    //获取CRC值
    crcDat = ConvertArrayToUint32HighFirst(uartCmdUtilPtr->cmdDat,0);
    //任务开始
    AppShowStartMsg(AppHeartDebugStartMsg,detailDebugMsgPtr,crcDat,0,0,0,0);
    //实际调用
    errorCode = ServiceImplDebugIapCalcCRC32(crcDat);
