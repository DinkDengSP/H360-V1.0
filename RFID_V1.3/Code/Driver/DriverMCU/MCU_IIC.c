...
**Author: DengXiaoJun
**Date: 2020-11-17 09:33:45
**LastEditors: DengXiaoJun
**LastEditTime: 2020-11-17 10:04:29
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
void MCU_IIC_PortInit(void)
{
    ...
    if(MCU_IIC_InitFlag == 0)
    {
        ...
        //SCL
        MCU_PortInit(MCU_PIN_B_8, GPIO_Mode_OUT, GPIO_OType_PP, GPIO_PuPd_UP, GPIO_Speed_Level_3);
        MCU_PortWriteSingle(MCU_PIN_B_8, Bit_SET);
        //SDA
        MCU_PortInit(MCU_PIN_B_9, GPIO_Mode_OUT, GPIO_OType_PP, GPIO_PuPd_UP, GPIO_Speed_Level_3);
...
LH_ERR MCU_IIC_WaitAck(void)
{
    ...
    while (SDA_IN_READ())
    {
        ...
        if (ucErrTime > 250)
        {
            //时间超时,返回超时
            MCU_IIC_Stop();
            return LH_ERR_IIC_ACK_TIME_OUT;
        }
    }
    //时钟输出0
    SCL_OUT_LOW();
