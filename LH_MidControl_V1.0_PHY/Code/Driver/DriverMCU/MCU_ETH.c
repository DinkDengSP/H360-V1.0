...
** Date          : 2022-08-08 13:45:14 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-09-14 10:15:14 +0800
************************************************************************************************/ 
#include "MCU_ETH.h"
#include "MCU_Port.h"
#include "MCU_RTT.h"
...
void HAL_ETH_DMAErrorCallback(ETH_HandleTypeDef *heth)
{
    if(heth->Instance == ETH)
    {
        MCU_RTT_SendStringInt("HAL_ETH_DMAErrorCallback\r\n");
    }
}
...
void HAL_ETH_MACErrorCallback(ETH_HandleTypeDef *heth)
{
    if(heth->Instance == ETH)
    {
        MCU_RTT_SendStringInt("HAL_ETH_MACErrorCallback\r\n");
    }
}
...
void HAL_ETH_PMTCallback(ETH_HandleTypeDef *heth)
{
    if(heth->Instance == ETH)
    {
        MCU_RTT_SendStringInt("HAL_ETH_PMTCallback\r\n");
    }
}
...
void HAL_ETH_EEECallback(ETH_HandleTypeDef *heth)
{
    if(heth->Instance == ETH)
    {
        MCU_RTT_SendStringInt("HAL_ETH_EEECallback\r\n");
    }
}
...
void HAL_ETH_WakeUpCallback(ETH_HandleTypeDef *heth)
{
    if(heth->Instance == ETH)
    {
        MCU_RTT_SendStringInt("HAL_ETH_WakeUpCallback\r\n");
    }
}
