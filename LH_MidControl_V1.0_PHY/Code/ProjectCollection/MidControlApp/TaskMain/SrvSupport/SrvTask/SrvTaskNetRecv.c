...
**LastEditTime: 2021-08-09 19:59:04
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
void TaskFuncSrvNetRecv(unsigned long threadInput)
{
    ...
    //从消息队列中获取到的消息指针
    NX_DATA_MAIN_PORT* netRecvPackMsgPtr = NULL;
    while(1)
    {
        if(netRecvPackMsgPtr != NULL)
        {
            //每次运行前释放内存
            if(netRecvPackMsgPtr->recvDatBufPtr != NULL)
            {
                UserMemFree(MEM_AXI_SRAM,netRecvPackMsgPtr->recvDatBufPtr);
            }
        ...
        {
            if(netRecvPackMsgPtr->recvDataLength == 0)
            {
                //接收到的数据长度为0,退出本次循环
                continue;
            }
            //打印接收数据
            SystemPrintf("[MID]NetRecv,DataLength:%d\r\n",netRecvPackMsgPtr->recvDataLength);
            //分析接收到的数据
            errorCode = SrvImplNetRecvWithProtocol(netRecvPackMsgPtr->recvDatBufPtr,netRecvPackMsgPtr->recvDataLength);
