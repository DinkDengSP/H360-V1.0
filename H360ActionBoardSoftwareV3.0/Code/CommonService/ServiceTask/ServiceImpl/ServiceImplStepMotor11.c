...
**Author: DengXiaoJun
**Date: 2020-02-10 09:59:14
**LastEditors: DengXiaoJun
**LastEditTime: 2020-05-08 09:08:39
**FilePath: \H360_ActionBoard_ID_2d:\SVN_Local\02-Development\Software\H360_STM32\H360ActionBoardSoftwareV2.3\CommonService\ServiceTask\ServiceImpl\ServiceImplStepMotor11.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
**ModifyRecord5:    
******************************************************************/
#include "ServiceImplStepMotor11.h"
...
LH_ERR ServiceImplStepMotor11ResetWithCanMsg(void* canMsgCachePtr)
{
    ...
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorResetCmdParamFillWithCanMsg(&resetCmd,cachePtrLocal,STEP_MOTOR11);
    //调用复位算法
...
LH_ERR ServiceImplStepMotor11RunSpecialStepsWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunSpecialStepsCmdParamFillWithCanMsg(&stepMotorRunStepsCmd,cachePtrLocal,STEP_MOTOR11);
...
LH_ERR ServiceImplStepMotor11RunToCoordinatesWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunToCoordinatesCmdParamFillWithCanMsg(&stepMotorRun2CoordationCmd,cachePtrLocal,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
...
LH_ERR ServiceImplStepMotor11ReturnZeroWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroCmdParamFillWithCanMsg(&stepMotorReturnZeroCmd,cachePtrLocal,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
...
LH_ERR ServiceImplStepMotor11ReturnZeroWithoutResetWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroWithOutResetCmdParamFillWithCanMsg(&stepMotorReturnZeroCmd,cachePtrLocal,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckReturnZeroVar(&stepMotorReturnZeroCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor11RunAlwaysWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunAlwaysCmdParamFillWithCanMsg(&stepMotorRunAlwaysCmd,cachePtrLocal,STEP_MOTOR11); 
...
LH_ERR ServiceImplStepMotor11ReadCurrentPositionWithCanMsg(void* canMsgCachePtr)
{
    ...
    coordinateTemp = *((uint32_t*)(&(stepMotorVar[STEP_MOTOR11]->var.currentCoordinate)));
...
LH_ERR ServiceImplStepMotor11ResetWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorResetCmd resetCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorResetCmdParamFillWithSelfMsg(&resetCmd,selfCmdPtr,STEP_MOTOR11);
    //调用复位算法
    errCode = StepMotorReset(&resetCmd);
    if(errCode != LH_ERR_NONE)
    {
        ...
        cmdResultPtr->currentCoordinate = stepMotorVar[STEP_MOTOR11]->var.currentCoordinate;
        return errCode;
    }
    //更新电机状态
    DATA_IPC_STEPMOTOR* cmdResultPtr = (DATA_IPC_STEPMOTOR*)(selfCmdPtr->commandDataSpecialChannel);
...
LH_ERR ServiceImplStepMotor11RunStepsWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorRunStepsCmd stepMotorRunStepsCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunSpecialStepsCmdParamFillWithSelfMsg(&stepMotorRunStepsCmd,selfCmdPtr,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckRunStepsVar(&stepMotorRunStepsCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor11Run2CoordinatesWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorRun2CoordationCmd stepMotorRun2CoordationCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunToCoordinatesCmdParamFillWithSelfMsg(&stepMotorRun2CoordationCmd,selfCmdPtr,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckRun2CoordationVar(&stepMotorRun2CoordationCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor11ReturnZeroWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorReturnZeroCmd stepMotorReturnZeroCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroCmdParamFillWithSelfMsg(&stepMotorReturnZeroCmd,selfCmdPtr,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckReturnZeroVar(&stepMotorReturnZeroCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor11ReturnZeroWithoutResetWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorReturnZeroCmd stepMotorReturnZeroCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroWithOutResetCmdParamFillWithSelfMsg(&stepMotorReturnZeroCmd,selfCmdPtr,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckReturnZeroVar(&stepMotorReturnZeroCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor11RunAlwaysWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorRunAlwaysCmd stepMotorRunAlwaysCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunAlwaysCmdParamFillWithSelfMsg(&stepMotorRunAlwaysCmd,selfCmdPtr,STEP_MOTOR11);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckRunAlways(&stepMotorRunAlwaysCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor11StopImmediatelyWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    //结果缓存
    LH_ERR errCode = LH_ERR_NONE;
    errCode = StepMotorStopImmediately(STEP_MOTOR11);
    return errCode;
}
...
LH_ERR ServiceImplStepMotor11StopDecelerationWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    //结果缓存
    LH_ERR errCode = LH_ERR_NONE;
    errCode = StepMotorStopDeceleration(STEP_MOTOR11);
    return errCode;
}
...
LH_ERR ServiceImplStepMotor11StopEmergencyWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    //结果缓存
    LH_ERR errCode = LH_ERR_NONE;
    errCode = StepMotorStopEmergency(STEP_MOTOR11);
    return errCode;
}
...
LH_ERR ServiceImplStepMotor11SetStateWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    //设置电机状态
    stepMotorVar[STEP_MOTOR11]->var.motorMainState = motorMainState;
    stepMotorVar[STEP_MOTOR11]->var.motorSubState  = motorSubState;
    return LH_ERR_NONE;
}
