...
**Author: DengXiaoJun
**Date: 2020-02-10 09:58:52
**LastEditors: DengXiaoJun
**LastEditTime: 2020-05-08 09:06:32
**FilePath: \H360_ActionBoard_ID_2d:\SVN_Local\02-Development\Software\H360_STM32\H360ActionBoardSoftwareV2.3\CommonService\ServiceTask\ServiceImpl\ServiceImplStepMotor7.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
**ModifyRecord5:    
******************************************************************/
#include "ServiceImplStepMotor7.h"
...
LH_ERR ServiceImplStepMotor7ResetWithCanMsg(void* canMsgCachePtr)
{
    ...
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorResetCmdParamFillWithCanMsg(&resetCmd,cachePtrLocal,STEP_MOTOR7);
    //调用复位算法
...
LH_ERR ServiceImplStepMotor7RunSpecialStepsWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunSpecialStepsCmdParamFillWithCanMsg(&stepMotorRunStepsCmd,cachePtrLocal,STEP_MOTOR7);
...
LH_ERR ServiceImplStepMotor7RunToCoordinatesWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunToCoordinatesCmdParamFillWithCanMsg(&stepMotorRun2CoordationCmd,cachePtrLocal,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
...
LH_ERR ServiceImplStepMotor7ReturnZeroWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroCmdParamFillWithCanMsg(&stepMotorReturnZeroCmd,cachePtrLocal,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
...
LH_ERR ServiceImplStepMotor7ReturnZeroWithoutResetWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroWithOutResetCmdParamFillWithCanMsg(&stepMotorReturnZeroCmd,cachePtrLocal,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckReturnZeroVar(&stepMotorReturnZeroCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor7RunAlwaysWithCanMsg(void* canMsgCachePtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunAlwaysCmdParamFillWithCanMsg(&stepMotorRunAlwaysCmd,cachePtrLocal,STEP_MOTOR7); 
...
LH_ERR ServiceImplStepMotor7ReadCurrentPositionWithCanMsg(void* canMsgCachePtr)
{
    ...
    coordinateTemp = *((uint32_t*)(&(stepMotorVar[STEP_MOTOR7]->var.currentCoordinate)));
...
LH_ERR ServiceImplStepMotor7ResetWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorResetCmd resetCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorResetCmdParamFillWithSelfMsg(&resetCmd,selfCmdPtr,STEP_MOTOR7);
    //调用复位算法
    errCode = StepMotorReset(&resetCmd);
    if(errCode != LH_ERR_NONE)
    {
        ...
        cmdResultPtr->currentCoordinate = stepMotorVar[STEP_MOTOR7]->var.currentCoordinate;
        return errCode;
    }
    //更新电机状态
    DATA_IPC_STEPMOTOR* cmdResultPtr = (DATA_IPC_STEPMOTOR*)(selfCmdPtr->commandDataSpecialChannel);
...
LH_ERR ServiceImplStepMotor7RunStepsWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorRunStepsCmd stepMotorRunStepsCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunSpecialStepsCmdParamFillWithSelfMsg(&stepMotorRunStepsCmd,selfCmdPtr,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckRunStepsVar(&stepMotorRunStepsCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor7Run2CoordinatesWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorRun2CoordationCmd stepMotorRun2CoordationCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunToCoordinatesCmdParamFillWithSelfMsg(&stepMotorRun2CoordationCmd,selfCmdPtr,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckRun2CoordationVar(&stepMotorRun2CoordationCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor7ReturnZeroWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorReturnZeroCmd stepMotorReturnZeroCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroCmdParamFillWithSelfMsg(&stepMotorReturnZeroCmd,selfCmdPtr,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckReturnZeroVar(&stepMotorReturnZeroCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor7ReturnZeroWithoutResetWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorReturnZeroCmd stepMotorReturnZeroCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorReturnZeroWithOutResetCmdParamFillWithSelfMsg(&stepMotorReturnZeroCmd,selfCmdPtr,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckReturnZeroVar(&stepMotorReturnZeroCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor7RunAlwaysWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    StepMotorRunAlwaysCmd stepMotorRunAlwaysCmd;
    //根据收到的数据填充结构体
    ServiceImplBaseStepMotorRunAlwaysCmdParamFillWithSelfMsg(&stepMotorRunAlwaysCmd,selfCmdPtr,STEP_MOTOR7);
    //检查参数列表.合规,就可以进行进一步的处理,否则,直接报错
    errCode = StepMotorCheckRunAlways(&stepMotorRunAlwaysCmd);
    if(errCode != LH_ERR_NONE)
    {
...
LH_ERR ServiceImplStepMotor7StopImmediatelyWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    //结果缓存
    LH_ERR errCode = LH_ERR_NONE;
    errCode = StepMotorStopImmediately(STEP_MOTOR7);
    return errCode;
}
...
LH_ERR ServiceImplStepMotor7StopDecelerationWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    //结果缓存
    LH_ERR errCode = LH_ERR_NONE;
    errCode = StepMotorStopDeceleration(STEP_MOTOR7);
    return errCode;
}
...
LH_ERR ServiceImplStepMotor7StopEmergencyWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    //结果缓存
    LH_ERR errCode = LH_ERR_NONE;
    errCode = StepMotorStopEmergency(STEP_MOTOR7);
    return errCode;
}
...
LH_ERR ServiceImplStepMotor7SetStateWithSelfMsg(SYSTEM_CMD_SELF* selfCmdPtr)
{
    ...
    //设置电机状态
    stepMotorVar[STEP_MOTOR7]->var.motorMainState = motorMainState;
    stepMotorVar[STEP_MOTOR7]->var.motorSubState  = motorSubState;
    return LH_ERR_NONE;
}
