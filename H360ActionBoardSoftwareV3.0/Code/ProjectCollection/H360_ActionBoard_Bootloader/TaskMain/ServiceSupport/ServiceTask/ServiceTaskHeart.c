...
//模块自身转发消息处理任务
static void ServiceTaskFuncHeart_Self_Proc(SYSTEM_CMD_SELF* selfCmd);
    ...
    while(1)
    {
        ...
        if ((taskCommandPtr != NULL)&&(err == OS_ERR_NONE))
		{
            ...
            else if(TASK_CMD_SRC_SELF == taskCommandPtr->cmdSrc)
            {
                ...
                //自身指令处理
                ServiceTaskFuncHeart_Self_Proc(selfCmd);
                //测试完成,释放自身申请的内存
...
/*********************************************自身定时任务处理*********************************************/
//模块定时执行任务
void ServiceTaskFuncHeart_Auto_Proc(void)
{
...
/************************************************CAN总线接收到的消息的处理******************************************************/
//模块CAN信息处理任务
void ServiceTaskFuncHeart_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsg)
{
    return;
}
...
/************************************************自身分层API调用的消息的处理**********************************/
//模块自身转发消息处理任务
void ServiceTaskFuncHeart_Self_Proc(SYSTEM_CMD_SELF* selfCmd)
{
    return;
}
