...
**ModifyRecord5:    
******************************************************************/
#include "AppImplMixReagentVersion1.h"
...
static LH_ERR AppImplMixReagentStartRotateMode1(void)
{
    ...
    //设置电机持续运转指令
        ...
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_1_VERSION1;
...
static LH_ERR AppImplMixReagentStartRotateMode2(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_2_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode3(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_3_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode4(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_4_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode5(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_5_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode6(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_6_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode7(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_7_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode8(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_8_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode9(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_9_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
static LH_ERR AppImplMixReagentStartRotateMode10(void)
{
    ...
    //设置电机持续运转指令
        IPC_StepMotorUtilRunAlwaysDataStructInit(&runAlwaysCmd);
        runAlwaysCmd.motorIndex = MIX_REAGENT_STEP_MOTOR_ROTATE_ID;
        runAlwaysCmd.selectConfig = S_CURVE_MIX_REAGENT_ROTATE_MODE_10_VERSION1;
        runAlwaysCmd.dir = BOARD_CPLD_MOTOR_DIR_CW;
    //电机开始持续运转
        errorCode = IPC_StepMotorRunAlwaysWhileReturn(&runAlwaysCmd);
        if(errorCode != LH_ERR_NONE)
...
//试剂混匀模块复位,带零位偏移
LH_ERR AppImplMixReagentResetVersion1(void)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //参数配置
        //参数辅助,用于系统参数读取
    ...
    //设置旋转电机为复位状态
        errorCode = IPC_StepMotorSetStateWhileReturn(MIX_REAGENT_STEP_MOTOR_ROTATE_ID,STEP_MOTOR_MAIN_STATE_STOP,STEP_MOTOR_STOP_SUB_NORMAL);
        if(errorCode != LH_ERR_NONE)
        {
...
//试剂混匀升起来
LH_ERR AppImplMixReagentUpVersion1(MIX_MODE mixMode)
{
    //模式修正
...
//试剂混匀升降降下去
LH_ERR AppImplMixReagentDownVersion1(void)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //参数辅助
    int32_t paramUtil;
...
//试剂混匀旋转混匀一次,不带自动升降
LH_ERR AppImplMixReagentStartRotateVersion1(MIX_MODE mixMode)
{
    //模式修正
    if(mixMode > MIX_MODE_MAX)
    {
...
//试剂混匀集成混匀一次
LH_ERR AppImplMixReagentRotateIntegratedVersion1(MIX_MODE mixMode)
{
    //模式修正
    if(mixMode > MIX_MODE_MAX)
    {
    ...
    CoreDelayMsSensitivity(MIX_REAHENT_VERSION1_NEED_WAIT_CUP_STABLE_MS);
    //调用实际的集成混匀的函数
...
//指定步进电机复位
LH_ERR AppImplMixReagentStepMotorResetVersion1(INDEX_MOTOR_MIX_REAGENT index)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //参数辅助
    int32_t paramUtil;
    //复位指令结构体
        ...
        IPC_StepMotorUtilResetCmdDataStructInit(&resetCmd);
        //旋转电机无原点，只设置电机状态。
        if(index == INDEX_MOTOR_MIX_REAGENT_ROTATE)
        {
...
//指定步进电机走指定步数
LH_ERR AppImplMixReagentStepMotorRunStepsVersion1(INDEX_MOTOR_MIX_REAGENT index,int32_t steps,int32_t* posAfterRun)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //走步数结构体
