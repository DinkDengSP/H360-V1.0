#include "AppIpcOther.h"
#include "AppTaskConfig.h"

//IPC调用的数据结构
static DATA_IPC_OTHER dataIPC_Other;
//互斥信号量,用于API的互斥访问
static OS_MUTEX mutexIPC_Other;

//指令列表
typedef enum IPC_CMD_OTHER
{
    IPC_CMD_OTHER_UNDEFINE = 0X0000,
}IPC_CMD_OTHER;
...
void IPC_OtherDataInit(void)
{
    OS_ERR err;
    //创建互斥信号量
    OSMutexCreate((OS_MUTEX*	)&mutexIPC_Other,
				  (CPU_CHAR*	)"mutexIPC_Other",
                  (OS_ERR*		)&err);	
    //状态赋值
    dataIPC_Other.dataMutexPtr = &mutexIPC_Other;
    dataIPC_Other.state = STATE_IPC_IDLE;
    dataIPC_Other.resultValue = LH_ERR_NONE;
}
...
STATE_IPC IPC_OtherGetLastCommandStatusAndResult(LH_ERR* resultCode)
{
    OS_ERR err;
    //申请信号量
    OSMutexPend (&mutexIPC_Other,0,OS_OPT_PEND_NON_BLOCKING,0,&err);
    if(err != OS_ERR_NONE)
    {
    ...
    //申请到了
    *resultCode = dataIPC_Other.resultValue;
    //释放信号量
    OSMutexPost(&mutexIPC_Other,OS_OPT_POST_NONE,&err);
    //返回动作完成
    return dataIPC_Other.state;   
}

//动作API发送任务给动作API
static LH_ERR IPC_OtherSendCommandWhileAck(uint16_t cmdIndex,int32_t param1,int32_t param2,int32_t param3,int32_t param4,int32_t param5)
{
    OS_ERR err;
    //系统消息指针
    ...
    dataIPC_Other.state = STATE_IPC_WAIT_ACK;
    //指令结果值设置为初始化状态
    dataIPC_Other.resultValue = LH_ERR_NONE;
    ...
    //设置不同信道特有的数据指针
    utilSelfCmdPtr->commandDataSpecialChannel = (void*)(&dataIPC_Other);
    
    //将申请的指针传递给对应的任务信道做处理
    OSTaskQPost((OS_TCB *)&tcbAppTaskOther, //向协议处理线程发送消息
                        (void *)systemTaskCmd,
    ...
    while(dataIPC_Other.state == STATE_IPC_WAIT_ACK)
    {
        CoreDelayMinTick();
    }
...
{
    return IPC_OtherSendCommandWhileAck(IPC_CMD_OTHER_UNDEFINE,0,0,0,0,0);
}
