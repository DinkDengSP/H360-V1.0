...
static void ServiceTaskFuncSerialUserRS1_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr)
{
    ...
    //函数调用
    errorCode = ServiceTaskCanMsgProcSerialUserRS1Setting[commandValue].funcPtr(canMsgCachePtr,
                    ServiceTaskCanMsgProcSerialUserRS1Setting[commandValue].debugDetailMsg);
    //返回结果代码
