...
static const OS_TASK_CREATE_CONFIG AppTaskArray[] = {
    {&tcbAppTaskStepMotor, NAME_APP_TASK_STEPMOTOR, AppTaskFuncStepMotor, 
        PRIO_APP_TASK_STEPMOTOR, &stackBufferAppTaskStepMotor[0],
        STK_SIZE_APP_TASK_STEPMOTOR / 10, STK_SIZE_APP_TASK_STEPMOTOR, 
        1},//工装软件步进电机测试
    {&tcbAppTaskInOut, NAME_APP_TASK_INOUT, AppTaskFuncInOut, 
        PRIO_APP_TASK_INOUT, &stackBufferAppTaskInOut[0],
        STK_SIZE_APP_TASK_INOUT / 10, STK_SIZE_APP_TASK_INOUT, 
        1},//输入输出检查任务
    {&tcbAppTaskBarCodeScan, NAME_APP_TASK_BARCODE_SCAN, AppTaskFuncBarCodeScan, 
        PRIO_APP_TASK_BARCODE_SCAN, &stackBufferAppTaskBarCodeScan[0],
        STK_SIZE_APP_TASK_BARCODE_SCAN / 10, STK_SIZE_APP_TASK_BARCODE_SCAN, 
        1},//条码枪扫描任务
};
//系统应用任务的数量,用于初始化
...
void BoardDeviceInit(void)
{
    ...
    do
    {
        ...
        if(boardid != SELF_BOARD_ID)
        {
            ...
            LowLevelPrintf("BoardID_Init Match Failed\r\n");
            CoreDelayMs(100);
        }
    }while(0);
