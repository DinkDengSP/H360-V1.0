...
#pragma pack(8)
//任务堆栈
CPU_STK stackBufferAppTaskBarCodeScan[STK_SIZE_APP_TASK_BARCODE_SCAN]@".ccram";;
#pragma pack()

//任务控制块
OS_TCB tcbAppTaskBarCodeScan@".ccram";


//模块定时执行任务
static void AppTaskFuncBarCodeScan_Auto_Proc(void);
//模块CAN信息处理任务
static void AppTaskFuncBarCodeScan_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsg);
//模块自身转发消息处理任务
static void AppTaskFuncBarCodeScan_Self_Proc(SYSTEM_CMD_SELF* selfCmdPtr);
...
void AppTaskFuncBarCodeScan(void *p_arg)
{
    ...
    while(1)
    {
		...
		if(currentStateCheckTick - lastStateCheckTick >= 1000)
		{
			...
			//模块定时执行任务
			AppTaskFuncBarCodeScan_Auto_Proc();
		}
        ...
        if ((taskCommandPtr != NULL)&&(err == OS_ERR_NONE))
		{
            ...
            if(TASK_CMD_SRC_CAN == taskCommandPtr->cmdSrc)
            {
                ...
                //对这个数据进行处理
                AppTaskFuncBarCodeScan_CanMsg_Proc(canRecvDatPtr);
                //处理完成
            ...
            else if(TASK_CMD_SRC_SELF == taskCommandPtr->cmdSrc)
            {
                ...
                //自身指令处理
                AppTaskFuncBarCodeScan_Self_Proc(selfCmdPtr);
                //测试完成,释放自身申请的内存
...
/************************************************CAN总线接收到的消息的处理******************************************************/
//基础任务结构,任务实际情况未知
static LH_ERR AppTaskFuncBarCodeScan_CanMsg_Proc_Undefine(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr);

//配置参数,代表系统回调列表以及参数需求
static const SYSTEM_TASK_CAN_MSG_PROC_UNIT AppTaskCanMsgProcBarCodeScanSetting[] = {
    /****前一个是处理函数***************************************参数的最小长度*****/
    {AppTaskFuncBarCodeScan_CanMsg_Proc_Undefine         ,           2           ,"" }
};
#define TASK_CAN_MSG_BARCODE_SCAN_COMMAND_COUNT  ((sizeof(AppTaskCanMsgProcBarCodeScanSetting))/(sizeof(AppTaskCanMsgProcBarCodeScanSetting[0])))

//模块CAN信息处理任务
static void AppTaskFuncBarCodeScan_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr)
{
    ...
    if(commandValue >= TASK_CAN_MSG_BARCODE_SCAN_COMMAND_COUNT)
    {
        //指令不支持
    ...
    //识别长度是否支持,前四个字节是指令码
    if((4 +AppTaskCanMsgProcBarCodeScanSetting[commandValue].paramDataMinLength) > canMsgCachePtr->currentLength)
    {
        //参数长度不够
    ...
    //函数调用
    errorCode = AppTaskCanMsgProcBarCodeScanSetting[commandValue].funcPtr(canMsgCachePtr,AppTaskCanMsgProcBarCodeScanSetting[commandValue].debugDetailMsg);
    //返回结果代码
...
static LH_ERR AppTaskFuncBarCodeScan_CanMsg_Proc_Undefine(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr)
{
    ...
    //写入数据
    AppImplBarCodeScanTestSupport(param1,param2);
    //返回结果
    return LH_ERR_NONE;
}
...
/************************************************自身分层API调用的消息的处理**********************************/
//基础任务结构,实际实现未知
static LH_ERR AppTaskFuncBarCodeScan_Self_Proc_Undefine(SYSTEM_CMD_SELF* selfCmdPtr,uint8_t* detailDebugMsgPtr);

//配置参数,代表系统回调列表以及参数需求
static const SYSTEM_TASK_SELF_MSG_PROC_UNIT AppTaskSelfMsgProcBarCodeScanSetting[] = {
    AppTaskFuncBarCodeScan_Self_Proc_Undefine,
};

//自身支持指令数量
#define TASK_SELF_MSG_BARCODE_SCAN_COMMAND_COUNT  ((sizeof(AppTaskSelfMsgProcBarCodeScanSetting))/(sizeof(AppTaskSelfMsgProcBarCodeScanSetting[0])))
...
static void AppTaskFuncBarCodeScan_Self_Proc(SYSTEM_CMD_SELF* selfCmdPtr)
{
	...
	//检查序号,不能大于最大支持
	if(selfCmdPtr->commandIndex >= TASK_SELF_MSG_BARCODE_SCAN_COMMAND_COUNT)
	{
       AppShowStartMsg((uint8_t*)AppBarCodeScanDebugStartMsg,(uint8_t*)"Unsupport",
	...
	{
        //调用相关指令处理
		actionResult = AppTaskSelfMsgProcBarCodeScanSetting[selfCmdPtr->commandIndex].funcPtr(selfCmdPtr,
                         AppTaskSelfMsgProcBarCodeScanSetting[selfCmdPtr->commandIndex].debugDetailMsg);
	}
    //模拟时序,假动作
    AppShowEndMsg((uint8_t*)AppBarCodeScanDebugEndMsg,actionResult);
...
static LH_ERR AppTaskFuncBarCodeScan_Self_Proc_Undefine(SYSTEM_CMD_SELF* selfCmdPtr,uint8_t* detailDebugMsgPtr)
{
    ...
    //动作开始,打印开始信息
    AppShowStartMsg((uint8_t*)AppBarCodeScanDebugStartMsg,(uint8_t*)"Undefine",selfCmdPtr->commandParam1,
