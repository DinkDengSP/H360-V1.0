...
#pragma pack(8)
//任务堆栈
CPU_STK stackBufferAppTaskStepMotor[STK_SIZE_APP_TASK_STEPMOTOR]@".ccram";;
#pragma pack()

//任务控制块
OS_TCB tcbAppTaskStepMotor@".ccram";


//模块定时执行任务
static void AppTaskFuncStepMotor_Auto_Proc(void);
//模块CAN信息处理任务
static void AppTaskFuncStepMotor_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsg);
//模块自身转发消息处理任务
static void AppTaskFuncStepMotor_Self_Proc(SYSTEM_CMD_SELF* selfCmdPtr);
...
void AppTaskFuncStepMotor(void *p_arg)
{
    ...
    while(1)
    {
		...
		if(currentStateCheckTick - lastStateCheckTick >= 1000)
		{
			...
			//模块定时执行任务
			AppTaskFuncStepMotor_Auto_Proc();
		}
        ...
        if ((taskCommandPtr != NULL)&&(err == OS_ERR_NONE))
		{
            ...
            if(TASK_CMD_SRC_CAN == taskCommandPtr->cmdSrc)
            {
                ...
                //对这个数据进行处理
                AppTaskFuncStepMotor_CanMsg_Proc(canRecvDatPtr);
                //处理完成
            ...
            else if(TASK_CMD_SRC_SELF == taskCommandPtr->cmdSrc)
            {
                ...
                //自身指令处理
                AppTaskFuncStepMotor_Self_Proc(selfCmdPtr);
                //测试完成,释放自身申请的内存
...
/************************************************CAN总线接收到的消息的处理******************************************************/
//基础任务结构,任务实际情况未知
static LH_ERR AppTaskFuncStepMotor_CanMsg_Proc_Undefine(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr);

//配置参数,代表系统回调列表以及参数需求
static const SYSTEM_TASK_CAN_MSG_PROC_UNIT AppTaskCanMsgProcStepMotorSetting[] = {
    /****前一个是处理函数***************************************参数的最小长度*****/
    {AppTaskFuncStepMotor_CanMsg_Proc_Undefine         ,           2           ,"" }
};
#define TASK_CAN_MSG_STEPMOTOR_COMMAND_COUNT  ((sizeof(AppTaskCanMsgProcStepMotorSetting))/(sizeof(AppTaskCanMsgProcStepMotorSetting[0])))

//模块CAN信息处理任务
static void AppTaskFuncStepMotor_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr)
{
    ...
    if(commandValue >= TASK_CAN_MSG_STEPMOTOR_COMMAND_COUNT)
    {
        //指令不支持
    ...
    //识别长度是否支持,前四个字节是指令码
    if((4 +AppTaskCanMsgProcStepMotorSetting[commandValue].paramDataMinLength) > canMsgCachePtr->currentLength)
    {
        //参数长度不够
    ...
    //函数调用
    errorCode = AppTaskCanMsgProcStepMotorSetting[commandValue].funcPtr(canMsgCachePtr,AppTaskCanMsgProcStepMotorSetting[commandValue].debugDetailMsg);
    //返回结果代码
...
static LH_ERR AppTaskFuncStepMotor_CanMsg_Proc_Undefine(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr)
{
    ...
    //写入数据
    AppImplStepMotorTestSupport(param1,param2);
    //返回结果
    return LH_ERR_NONE;
}
...
/************************************************自身分层API调用的消息的处理**********************************/
//基础任务结构,实际实现未知
static LH_ERR AppTaskFuncStepMotor_Self_Proc_Undefine(SYSTEM_CMD_SELF* selfCmdPtr,uint8_t* detailDebugMsgPtr);

//配置参数,代表系统回调列表以及参数需求
static const SYSTEM_TASK_SELF_MSG_PROC_UNIT AppTaskSelfMsgProcStepMotorSetting[] = {
    AppTaskFuncStepMotor_Self_Proc_Undefine,
};

//自身支持指令数量
#define TASK_SELF_MSG_STEPMOTOR_COMMAND_COUNT  ((sizeof(AppTaskSelfMsgProcStepMotorSetting))/(sizeof(AppTaskSelfMsgProcStepMotorSetting[0])))
...
static void AppTaskFuncStepMotor_Self_Proc(SYSTEM_CMD_SELF* selfCmdPtr)
{
	...
	//检查序号,不能大于最大支持
	if(selfCmdPtr->commandIndex >= TASK_SELF_MSG_STEPMOTOR_COMMAND_COUNT)
	{
       AppShowStartMsg((uint8_t*)AppStepMotorDebugStartMsg,(uint8_t*)"Unsupport",
	...
	{
        //调用相关指令处理
		actionResult = AppTaskSelfMsgProcStepMotorSetting[selfCmdPtr->commandIndex].funcPtr(selfCmdPtr,
                         AppTaskSelfMsgProcStepMotorSetting[selfCmdPtr->commandIndex].debugDetailMsg);
	}
    //模拟时序,假动作
    AppShowEndMsg((uint8_t*)AppStepMotorDebugEndMsg,actionResult);
...
static LH_ERR AppTaskFuncStepMotor_Self_Proc_Undefine(SYSTEM_CMD_SELF* selfCmdPtr,uint8_t* detailDebugMsgPtr)
{
    ...
    //动作开始,打印开始信息
    AppShowStartMsg((uint8_t*)AppStepMotorDebugStartMsg,(uint8_t*)"Undefine",selfCmdPtr->commandParam1,
