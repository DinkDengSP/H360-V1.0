...
//读取流量计脉冲
static LH_ERR ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ReadLiquidCount(void* canMsgCachePtr,uint8_t*  detailDebugMsgPtr);
//清除流量计脉冲信号
static LH_ERR ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ClearLiquidCount(void* canMsgCachePtr,uint8_t*  detailDebugMsgPtr);
//注册流量计脉冲讯号,当脉冲讯号到位的时候,立即关闭设定的输出
static LH_ERR ServiceTaskFuncSerialUserRS1_CanMsg_Proc_RegisterLiquidCount(void* canMsgCachePtr,uint8_t*  detailDebugMsgPtr);
//读取脉冲关闭讯号是否触发
static LH_ERR ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ReadLiquidCountTrigFlag(void* canMsgCachePtr,uint8_t*  detailDebugMsgPtr);
...
static const SYSTEM_TASK_CAN_MSG_PROC_UNIT ServiceTaskCanMsgProcSerialUserRS1Setting[] = {
    ...
    {ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ReadLiquidCount                 ,0    ,"" },
    {ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ClearLiquidCount                ,0    ,"" },
    {ServiceTaskFuncSerialUserRS1_CanMsg_Proc_RegisterLiquidCount             ,4    ,"" },
    {ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ReadLiquidCountTrigFlag         ,0    ,"" },
};
...
static LH_ERR ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ReadLiquidCount(void* canMsgCachePtr,uint8_t*  detailDebugMsgPtr)
{
    ...
    //液路触发标志
    returnResult = liquidSensorCount;
    //返回数据
    uint8_t dataBufferSend[4] = {0};
    CanBaseWriteUint32DataToBuffer(dataBufferSend,0,returnResult);
    //返回数据包
    CanSlaveSendDataPackWithRecvCache(cachePtrLocal,CAN_BASE_DATA_SERIAL_RS1,dataBufferSend,4);
    return LH_ERR_NONE;
}
...
static LH_ERR ServiceTaskFuncSerialUserRS1_CanMsg_Proc_RegisterLiquidCount(void* canMsgCachePtr,uint8_t*  detailDebugMsgPtr)
{
    ...
    CAN_SLAVE_CHANNEL_CACHE* cachePtrLocal = (CAN_SLAVE_CHANNEL_CACHE*)canMsgCachePtr;
    //计数器
    liquidSensorTargetCount = CanBaseReadDataConvertToUint32(cachePtrLocal->recvCacheArray,4);
    if(liquidSensorTargetCount == 0)
    {
        //计数值不能为0
        return LH_ERR_SERVICE_SERIAL_SEND_LENGTH_ZERO;
    }
    //注册
    liquidSensorCount = 0;
    trigFlag = 0;
...
static LH_ERR ServiceTaskFuncSerialUserRS1_CanMsg_Proc_ReadLiquidCountTrigFlag(void* canMsgCachePtr,uint8_t*  detailDebugMsgPtr)
{
    ...
    {
        //流量计未触发
        errorCode = LH_ERR_SERVICE_SERIAL_LIQUID_TRIG_NONE;
    }
    else
    {
        //流量计已经触发
