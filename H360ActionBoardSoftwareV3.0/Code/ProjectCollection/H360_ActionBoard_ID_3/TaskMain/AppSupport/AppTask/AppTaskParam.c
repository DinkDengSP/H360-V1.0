...
**Author: DengXiaoJun
**Date: 2020-02-11 14:38:01
**LastEditors: DengXiaoJun
**LastEditTime: 2020-03-07 15:20:46
**FilePath: \H360_ActionBoard_ID_3\TaskMain\AppSupport\AppTask\AppTaskParam.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
static const SYSTEM_TASK_CAN_MSG_PROC_UNIT AppTaskCanMsgProcFuncPtrArrayParam[] = {
    /****前一个是处理函数***************************************参数的最小长度*****/
    {AppTaskFuncParam_CanMsg_Proc_Read         ,2   ,"Read"         },//读取指定序号参数
    {AppTaskFuncParam_CanMsg_Proc_Write        ,6   ,"Write"        },//写入指定序号参数
    {AppTaskFuncParam_CanMsg_Proc_SetDefault   ,0   ,"SerDefault"   },//恢复默认参数
};
...
static void AppTaskFuncParam_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr)
{
    ...
    //函数调用
    actionResult = AppTaskCanMsgProcFuncPtrArrayParam[commandValue].funcPtr(canMsgCachePtr,
                                                                            AppTaskCanMsgProcFuncPtrArrayParam[commandValue].debugDetailMsg);
    //打印结果
    AppShowEndMsg((uint8_t*)AppParamDebugEndMsg,actionResult);
    //返回结果代码
    CanSlaveSendResultWithRecvCache(canMsgCachePtr,actionResult);
}
...
static LH_ERR AppTaskFuncParam_CanMsg_Proc_Read(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr)
{
    ...
    INDEX_CAN_SUB_BOARD3_PARAM_MAIN mainIndex = (INDEX_CAN_SUB_BOARD3_PARAM_MAIN)(cachePtrLocal->recvCacheArray[4]);
    ...
    //调用复位函数
    actionResult = AppImplReadParam(mainIndex,subIndex,&paramRead);
...
static LH_ERR AppTaskFuncParam_CanMsg_Proc_Write(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr)
{
    ...
    //调用复位函数
    actionResult = AppImplWriteParam(mainIndex,subIndex,paramWrite);
    //返回结果
    return actionResult;
}
//初始化板载参数为默认值
...
static const SYSTEM_TASK_SELF_MSG_PROC_UNIT AppTaskSelfMsgProcFuncPtrArrayParam[] = {
    {AppTaskFuncParam_Self_Proc_SetDefault  ,"SetDefault" },//恢复默认参数
};
...
static void AppTaskFuncParam_Self_Proc(SYSTEM_CMD_SELF* selfCmdPtr)
{
	...
	{
        //调用相关指令处理
		actionResult = AppTaskSelfMsgProcFuncPtrArrayParam[selfCmdPtr->commandIndex].funcPtr(selfCmdPtr,
                                                                                                AppTaskSelfMsgProcFuncPtrArrayParam[selfCmdPtr->commandIndex].debugDetailMsg);
	}
    //模拟时序,假动作
    AppShowEndMsg((uint8_t*)AppParamDebugEndMsg,actionResult);
