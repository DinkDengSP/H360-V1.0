...
** Date          : 2022-09-15 09:29:46 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-09-16 10:01:48 +0800
************************************************************************************************/ 
#include "MCU_Flash.h"
...
//擦除指定扇区
static LH_ERR MCU_FlashEraseSector(uint8_t bankNo,uint32_t sectorNo)
{
    ...
    if(flashState != HAL_OK)
	{
        ...
        return LH_ERR_MCU_FLASH_ERASE_CMD;
    }    
    //没有异常,等待擦除操作完成
    flashState = FLASH_WaitForLastOperation(MCU_FLASH_WAITE_TIME,bankNo);   
    if(flashState != HAL_OK)
	{
        //Flash重新上锁
        HAL_FLASH_Lock();
        ...
        //日志显示
        LowLevelShowHalError(flashState);
        return LH_ERR_MCU_FLASH_ERASE_TIME_OUT;
    }    
    //Flash重新上锁
    HAL_FLASH_Lock();
    /* 开中断 */
...
//FLASH写入,不带擦除
static LH_ERR MCU_FlashWriteNoCheck(uint32_t startAddr,uint32_t *bufferPtr,uint32_t dwordNumber)
{
    HAL_StatusTypeDef flashState;
    //地址计数辅助          
    ...
    while(addressUtil < endAddr)
    {
        ...
        if(flashState != HAL_OK)
	    {
            ...
            //日志显示
            LowLevelShowHalError(flashState);
            return LH_ERR_MCU_FLASH_PAGE_WRITE;
        }    
        addressUtil += 32;
        bufferPtr += 8;
    } 
...
//擦除指定地址的扇区，外部需要关闭中断
LH_ERR MCU_FlashEraseSectorByAddr(uint32_t flashAddr)
{
    //计算bank
    uint8_t bankNo = 0;
    //计算扇区号码
...
LH_ERR MCU_FlashEraseRangeWithCheck(uint32_t startAddr,uint32_t dwordNumber)
{
    ...
    while(addressUtil<endAddr)
    {
        ...
        if(MCU_FlashReadWord(addressUtil) != 0XFFFFFFFF)
        { 
            errorCodeSub = LH_ERR_MCU_FLASH_ERASE_CHECK;
            LowLevelShowErrorSubCode(errorCodeSub);
            return errorCodeSub;
        }
        addressUtil+=4;
...
//从指定地址开始写入指定长度的数据  	  
LH_ERR MCU_FlashWrite(uint32_t startAddr,uint32_t *bufferPtr,uint32_t dwordNumber)
{
    LH_ERR errorCodeSub = LH_ERR_NONE;
    //非法地址
    if((startAddr < MCU_FLASH_BASE)||(startAddr%4))
    {
        errorCodeSub = LH_ERR_MCU_FLASH_ADDR_LIMIT;
        LowLevelShowErrorSubCode(errorCodeSub);
        return errorCodeSub;
    }
    //擦除指定区域
