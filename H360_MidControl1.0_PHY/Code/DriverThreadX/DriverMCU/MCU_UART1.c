...
** Date          : 2022-09-15 09:12:57 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-09-16 11:06:21 +0800
************************************************************************************************/ 
#include "MCU_UART1.h"
#include "MCU_Port.h"
#include "MCU_RTT.h"
...
#pragma pack(4)
//用于当前发送的数据块
static uint8_t blockBufferWithUart1SendCurrent[LENGTH_UART1_BUFFER_WITH_BLOCK]@".d2_sram1_dma" = {0};
//用于循环队列存储数据的数组
static uint8_t bufferWithUart1SendQueue[LENGTH_UART1_BUFFER_WITH_SEND]@".d2_sram1_dma" = {0};
#pragma pack (pop)
#endif
...
//接收缓存
static UART_MSG_RECV uart1MsgRecvArray[COUNT_UART1_RECV_BLOCK];
#endif
//中断接收需要的数据块
...
//接收缓存
static uint8_t blockBufferWithUart1Recv[COUNT_UART1_RECV_BLOCK][LENGTH_UART1_RECV_SINGLE_BLOCK]@".d2_sram1_dma" = {0};
//当前接收对象
static uint8_t currentBufferWithUart1Recv[LENGTH_UART1_RECV_SINGLE_BLOCK]@".d2_sram1_dma" = {0};
#endif  
...
static void MCU_UART1_MspInit(void)
{
    ...
    MCU_PortInit(UART1_PIN_TX,MCU_PORT_MODE_AF_PP,MCU_PORT_PULL_PULLUP,MCU_PORT_SPEED_FREQ_HIGH,GPIO_AF7_USART1);
#endif
#if((MCU_UART1_TRANS_MODE == UART_MODE_TX_RX)||(MCU_UART1_TRANS_MODE == UART_MODE_RX))
    MCU_PortInit(UART1_PIN_RX,MCU_PORT_MODE_AF_PP,MCU_PORT_PULL_PULLUP,MCU_PORT_SPEED_FREQ_HIGH,GPIO_AF7_USART1);
#endif
}
...
{
#if((MCU_UART1_TRANS_MODE == UART_MODE_TX_RX)||(MCU_UART1_TRANS_MODE == UART_MODE_TX))
    MCU_PortDeInit(UART1_PIN_TX);
#endif
#if((MCU_UART1_TRANS_MODE == UART_MODE_TX_RX)||(MCU_UART1_TRANS_MODE == UART_MODE_RX))
    MCU_PortDeInit(UART1_PIN_RX);
#endif
    //关闭中断
...
void MCU_UART1_RecvDataInit(void)
{
    ...
    {
        uart1MsgRecvArray[indexUtil].msgSrc = UART_MSG_SRC_1;
        uart1MsgRecvArray[indexUtil].uartRecvDataBuffer = (uint8_t*)(blockBufferWithUart1Recv[indexUtil]);
        uart1MsgRecvArray[indexUtil].uartRecvDataLength = 0;
    }
    //队列导入
    for(indexUtil = 0;indexUtil < COUNT_UART1_RECV_BLOCK;indexUtil++)
    {
        ADT_CircularPtrQueueWrite(&circularPtrQueueUart1RecvIdleBlock,(void*)(&(uart1MsgRecvArray[indexUtil])));
    }
    //清除当前接收队列
...
static void MCU_UART1_StartRecv(void)
{
    ...
    HAL_UART_Receive_IT(&UART1_Handler, (uint8_t *)(currentBufferWithUart1Recv), LENGTH_UART1_RECV_SINGLE_BLOCK);
#endif
#if(MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA)
    //开始DMA接收中断
    HAL_UART_Receive_DMA(&UART1_Handler,(uint8_t *)(currentBufferWithUart1Recv), LENGTH_UART1_RECV_SINGLE_BLOCK);
#endif
}
...
//串口接收到的数据释放
void MCU_UART1_RecvDatRelease(UART_MSG_RECV* uartMsgRecvPtr)
{
    asm("nop");
#if((MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)||(MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA))
    ...
    UserMemClear((void*)(uartMsgRecvPtr->uartRecvDataBuffer),LENGTH_UART1_RECV_SINGLE_BLOCK);
    //数据指针写回
    ADT_CircularPtrQueueWrite(&circularPtrQueueUart1RecvIdleBlock,uartMsgRecvPtr);
#endif
}
...
void MCU_UART1_RxCpltCallback(void)
{
...
#if((MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)||(MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA))
    //从队列中拿出一个指针做接收转发
    UART_MSG_RECV* recvDispatchPtr = ADT_CircularPtrQueueGet(&circularPtrQueueUart1RecvIdleBlock);
    ...
    {
        recvDispatchPtr->msgSrc = UART_MSG_SRC_1;
        recvDispatchPtr->uartRecvDataLength = LENGTH_UART1_RECV_SINGLE_BLOCK;
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->uartRecvDataBuffer),(void*)currentBufferWithUart1Recv,
                        LENGTH_UART1_RECV_SINGLE_BLOCK);
        //数据转发
        if(uart1RecvFuncPtr != NULL)
        {
            uart1RecvFuncPtr(recvDispatchPtr);
        }
        else
        {
            //转发失败,清空
            recvDispatchPtr->uartRecvDataLength = 0;
            UserMemClear((void*)(recvDispatchPtr->uartRecvDataBuffer),LENGTH_UART1_RECV_SINGLE_BLOCK);
            //写回空闲队列
            ADT_CircularPtrQueueWrite(&circularPtrQueueUart1RecvIdleBlock,recvDispatchPtr);
        }
    }
    //清空接收
    UserMemClear((void*)currentBufferWithUart1Recv,LENGTH_UART1_RECV_SINGLE_BLOCK);
...
void MCU_UART1_IDLE_CallBack(void)
{
    //MCU_RTT_SendStringInt("MCU_UART1_IDLE_CallBack\r\n");
#if(MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
    //获取接收到了多少数据
    uint16_t recvDataLength = LENGTH_UART1_RECV_SINGLE_BLOCK - UART1_Handler.RxXferCount;
    //关闭上次接收
    HAL_UART_AbortReceive(&UART1_Handler);
    if((recvDataLength == 0)||(uart1RecvFuncPtr == NULL))
    {
        //清空接收
        UserMemClear((void*)currentBufferWithUart1Recv,LENGTH_UART1_RECV_SINGLE_BLOCK);
        //使能串口空闲中断
    ...
    else
    {
        ...
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->uartRecvDataBuffer),(void*)currentBufferWithUart1Recv,recvDataLength);
        //数据转发
        uart1RecvFuncPtr(recvDispatchPtr);
    }
    //清空接收
    ...
    // 计算接收到的数据长度
    uint8_t recvDataLength  = LENGTH_UART1_RECV_SINGLE_BLOCK - __HAL_DMA_GET_COUNTER(&UART1_RxDMA_Handler);   
    //关闭上次接收
    HAL_UART_AbortReceive(&UART1_Handler);
    if((recvDataLength == 0)||(uart1RecvFuncPtr == NULL))
    {
...
{
    //清空错误
    MCU_RTT_SendStringInt("MCU_UART1_ErrorCallback\r\n");
}
...
void USART1_IRQHandler(void) //串口1中断服务程序
{
    ...
    //检查空闲中断
    if(RESET != __HAL_UART_GET_FLAG(&UART1_Handler, UART_FLAG_IDLE))
    {
        //清除空闲中断标志（否则会一直不断进入中断）
        __HAL_UART_CLEAR_IDLEFLAG(&UART1_Handler);
        //调用空闲中断处理函数
        MCU_UART1_IDLE_CallBack();
    }
    //调用HAL库中断
