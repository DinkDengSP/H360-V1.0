...
** Date          : 2022-09-15 09:28:25 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-09-16 15:20:03 +0800
************************************************************************************************/ 
#include "MCU_SPI2.h"
#include "MCU_Port.h"
...
LH_ERR MCU_SPI2_ReadWriteBuffer(uint8_t* writeDataPtr,uint8_t* readDataPtr,uint16_t dataLength)
{
    ...
    if(transState == SPI_TRANS_STATE_START)
    {
        ...
        MCU_SPI2_ReleaseLock();
        //错误日志打印
        LowLevelShowErrorSubCode(ERROR_SUB_MCU_SPI2_TRANS_TIME_OUT);
        return ERROR_SUB_MCU_SPI2_TRANS_TIME_OUT;
    }
    ...
    else if(transState == SPI_TRANS_STATE_ERROR)
    {
        ...
        MCU_SPI2_ReleaseLock();
        //错误日志打印
        LowLevelShowErrorSubCode(ERROR_SUB_MCU_SPI2_TRANS_IT_ERR);
...
LH_ERR MCU_SPI2_ReadWriteBuffer(uint8_t* writeDataPtr,uint8_t* readDataPtr,uint16_t dataLength)
{
    ...
    for(indexUtil = 0; indexUtil < sendPackCountMax;indexUtil++)
    {
        ...
        if(halState != HAL_OK)
        {
            ...
            MCU_SPI2_ReleaseLock();
            //错误日志打印
            LowLevelShowErrorSubCode(ERROR_SUB_MCU_SPI2_TRANS_DMA_START);
        ...
        if(eventFlagsActualFlag&EVENT_FLAG_SPI2_ERROR_COMPLETE)
        {
            ...
            MCU_SPI2_ReleaseLock();
            //错误日志打印
            LowLevelShowErrorSubCode(ERROR_SUB_MCU_SPI2_TRANS_DMA_ERR);
