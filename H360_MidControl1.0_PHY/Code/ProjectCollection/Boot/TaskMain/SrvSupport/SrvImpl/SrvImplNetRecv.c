...
** Date          : 2022-09-15 11:05:03 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-09-19 14:43:11 +0800
************************************************************************************************/ 
...
LH_ERR SrvImplNetRecvWithProtocol(uint8_t* recvDataPtr, uint16_t size)
{
    ...
    //判定一个数据包的两帧数据是否超时
    if(netCmdRecvStatePtr->packLastRecvTimeTick != 0 && (currentTimeTick-netCmdRecvStatePtr->packLastRecvTimeTick) > 
            TCPIP_CMD_PACK_FRAME_TIMEOUT_MS)
    {
        //打印数据异常
        SystemPrintf("[MID]E>TcpCmd Reset:%d,%d\r\n", currentTimeTick, netCmdRecvStatePtr->packLastRecvTimeTick);
        //清理接收状态,当成一包新数据接收
    ...
    if(netCmdRecvStatePtr->recvDataLength+size < TCP_CMD_HEADER_SIZE)
    {
        ...
        UserMemCopy((void*)(netCmdRecvStatePtr->recvDat+netCmdRecvStatePtr->recvDataLength), recvDataPtr, size);
        //储存数据量增加
    ...
    else
    {
        ...
        do
        {
            ...
            if (netCmdRecvStatePtr->recvDataLength < TCP_CMD_HEADER_SIZE)
            {
                ...
                //拷贝指定长度的数据量
                UserMemCopy((void*)(netCmdRecvStatePtr->recvDat+netCmdRecvStatePtr->recvDataLength), recvDataPtr+recvDataProcessIndex, 
                                copySizeUtil);
                //已经拷贝的数据量增加
            ...
            if (netCmdRecvStatePtr->recvDataLength >= TCP_CMD_HEADER_SIZE)
            {
                ...
                {
                    //计算该指令应该收到多少的包体数据
                    netCmdRecvStatePtr->recvBodyLength = TcpBaseUtilConvertBufferToUint16(netCmdRecvStatePtr->recvDat,
                                TCP_CMD_HEADER_DLEN_OFFSET);
                    if (netCmdRecvStatePtr->recvBodyLength > 0)
                    {
                        //拷贝包体数据,先检查当前接收到的数据够不够填充包体,能填充全部拷贝,否则拷贝剩余部分
                        copySizeUtil = (netCmdRecvStatePtr->recvBodyLength+recvDataProcessIndex) <= size ? 
                                            netCmdRecvStatePtr->recvBodyLength : (size-recvDataProcessIndex);
                        ...
                        if(netCmdRecvStatePtr->recvDataLength - TCP_CMD_HEADER_SIZE > 0)
                        {
                            ...
                            copySizeUtilMax =  (netCmdRecvStatePtr->recvBodyLength + TCP_CMD_HEADER_SIZE) - 
                                                    netCmdRecvStatePtr->recvDataLength;
                            //如果拷贝数据大于最大能拷贝的数据,那么以最大拷贝数据为准
                            if(copySizeUtil > copySizeUtilMax)copySizeUtil = copySizeUtilMax;
                        }
                        UserMemCopy((void*)(netCmdRecvStatePtr->recvDat+netCmdRecvStatePtr->recvDataLength), 
                                            recvDataPtr+recvDataProcessIndex, copySizeUtil);
                        //一定要记得将已经处理的数据量增加
                        recvDataProcessIndex += copySizeUtil;
                        netCmdRecvStatePtr->recvDataLength += copySizeUtil;
                    }
                ...
                else
                {
                    ...
                    SystemPrintf("[MID]E>LeaderCode:%04X\r\n", cmdLeaderCode);
                    for (uint16_t i=0; i<size; i++)
                    {
                        SystemPrintf("[MID]%02X ", recvDataPtr[i]);
                    }
                    SystemPrintf("\r\n=============\r\n");
                    //清理接收状态,后面的数据当成一包新数据接收
                    TcpBaseUtilClearRecvState();
                    return LH_ERR_SRV_COMM_NET_LEADER;                     
                }
            }
            else
            {
...
LH_ERR SrvImplNetRecvWithCompletePack(NET_CMD_RECV_STATE* recvCompletePack)
{
    ...
    //申请内存,存放指令包
    netCommandPackPtr = UserMemMalloc(MEM_REGION_SRV,(sizeof(NET_COMMAND_PACK)/sizeof(uint8_t)));
    if(netCommandPackPtr == NULL)
    {
        SystemPrintf("[MID]E>Func:%s,Line:%d ,UserMemMalloc Failed\r\n",__FUNCTION__,__LINE__);
        return LH_ERR_SRV_COMM_NET_MALLOC_FAILED;
    }
    //指令包初始化
    ...
    {
        SystemPrintf("[MID]E>Func:%s,Line:%d ,headerDataCheckSum:%04X,%04X\r\n",__FUNCTION__,__LINE__,checkSumUtil, netCommandPackPtr->headerDataCheckSum);
        //释放内存
        UserMemFree(MEM_REGION_SRV,netCommandPackPtr);
        //回传NACK,让上位机重发
        TcpBaseSendAck(TCP_NET_ACK_ERROR);
        return LH_ERR_SRV_COMM_NET_CRC_HEAD;
    }
    //进行数据体的校验
    //计算数据包体校验和
    checkSumUtil = 0;    
    ...
    {
        SystemPrintf("[MID]E>Func:%s,Line:%d ,bodyDataCheckSum :%08X,%08X\r\n",__FUNCTION__,__LINE__,checkSumUtil, netCommandPackPtr->bodyDataCheckSum);
        //释放内存
        UserMemFree(MEM_REGION_SRV,netCommandPackPtr);
        //回传NACK,让上位机重发
        TcpBaseSendAck(TCP_NET_ACK_ERROR);
        return LH_ERR_SRV_COMM_NET_CRC_BODY;
    }
    //申请数据域指针
    if(netCommandPackPtr->bodyDataLength != 0)
    {
        netCommandPackPtr->bodyData = UserMemMalloc(MEM_REGION_SRV,netCommandPackPtr->bodyDataLength);
    ...
    //校验和过关,进行数据体拷贝
    UserMemCopy((void*)netCommandPackPtr->bodyData,recvCompletePack->recvDat+TCP_CMD_HEADER_SIZE,netCommandPackPtr->bodyDataLength);
    //包体数据构造完成,现在需要转发
    ...
    if(errorCode != LH_ERR_NONE)
    {
        ...
        if(netCommandPackPtr->bodyDataLength != 0)
        {
            UserMemFree(MEM_REGION_SRV,netCommandPackPtr->bodyData);
        }
        UserMemFree(MEM_REGION_SRV,netCommandPackPtr);
    }
...
LH_ERR  SrvImplNetRecvDispatchCommand(NET_COMMAND_PACK* commandPackPtr)
{
    ...
    if(commandPackPtr->netCommand == TCP_CMD_COMMUNICATE_ACK)
    {
        ...
        {
            //释放数据包
            UserMemFree(MEM_REGION_SRV,commandPackPtr->bodyData);
        }
        UserMemFree(MEM_REGION_SRV,commandPackPtr);
        return LH_ERR_NONE;
    }
    ...
    {
        SystemPrintf("[MID]E>Func:%s,Line:%d ,QueueSend Failed,txResult: %d\r\n",__FUNCTION__,__LINE__,txResult);
        return LH_ERR_SRV_COMM_NET_DISPATCH_ERR;
    }
    return LH_ERR_NONE;
}
...
{
    //日志显示相关讯息
    SystemPrintf("LAN8700 MainPort RecvDataPack,IP:%d.%d.%d.%d ,Port: %d,Length: %d\r\n",
                    DWORD_TO_BYTE_HH(nxDataRecvPtr->peerIpAddress),DWORD_TO_BYTE_HL(nxDataRecvPtr->peerIpAddress),
                    DWORD_TO_BYTE_LH(nxDataRecvPtr->peerIpAddress),DWORD_TO_BYTE_LL(nxDataRecvPtr->peerIpAddress),
                    nxDataRecvPtr->peerPort,nxDataRecvPtr->recvDataLength);
    //数据处理
    SrvImplNetRecvWithProtocol(nxDataRecvPtr->recvDatBufPtr,nxDataRecvPtr->recvDataLength);
}
...
{
    //日志显示相关讯息
    SystemPrintf("W5500 MainPort RecvDataPack,IP:%d.%d.%d.%d ,Port: %d,Length: %d\r\n",
                    DWORD_TO_BYTE_HH(nxDataRecvPtr->peerIpAddress),DWORD_TO_BYTE_HL(nxDataRecvPtr->peerIpAddress),
                    DWORD_TO_BYTE_LH(nxDataRecvPtr->peerIpAddress),DWORD_TO_BYTE_LL(nxDataRecvPtr->peerIpAddress),
                    nxDataRecvPtr->peerPort,nxDataRecvPtr->recvDataLength);
    //数据处理
