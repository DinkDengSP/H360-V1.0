...
**Author: DengXiaoJun
**Date: 2020-01-17 09:35:09
**LastEditors: DengXiaoJun
**LastEditTime: 2020-05-03 14:20:13
**FilePath: \LH_TC_APP_V2.3\TaskMain\ServiceSupport\ServiceTask\ServiceTaskParam.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
//自动运行程序的执行周期,单位毫秒
#define SERVICE_TASK_PARAM_AUTO_FUNC_RUNNING_PERIOD_MS            200
//系统循环自动程序初始化
static void TaskServiceParam_Auto_Init(void);
//系统还处理一些循环自动化程序
static void TaskServiceParam_Auto_Proc(void);


//系统服务任务接收几个方面的消息
//1. 来自CAN1的消息
static void TaskFuncServiceParam_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr);
//2. 来自自身的消息
static void TaskFuncServiceParam_SelfMsg_Proc(SYSTEM_CMD_SELF* selfCmdPtr);
...
void ServiceTaskFuncParam(void *p_arg)
{
    ...
    //状态机初始化
    TaskServiceParam_Auto_Init();
    while(1)
    {
        //更新定时时钟
        currentStateCheckTick = OSTimeGet(&err);
		if(currentStateCheckTick - lastStateCheckTick >= SERVICE_TASK_PARAM_AUTO_FUNC_RUNNING_PERIOD_MS)
		{
			//上次发送时间和本次发送时间差了SERVICE_TASK_PARAM_AUTO_FUNC_RUNNING_PERIOD_MS个时间
			lastStateCheckTick = currentStateCheckTick;
			//模块定时执行任务
			TaskServiceParam_Auto_Proc();
		}
        ...
        taskCommandPtr = OSTaskQPend((OS_TICK)SERVICE_TASK_PARAM_AUTO_FUNC_RUNNING_PERIOD_MS,
        ...
        if ((taskCommandPtr != NULL)&&(err == OS_ERR_NONE))
		{
            ...
            if(TASK_CMD_SRC_CAN == taskCommandPtr->cmdSrc)
            {
                ...
                //对这个数据进行处理
                TaskFuncServiceParam_CanMsg_Proc(canRecvDatPtr);
                //处理完成
            ...
            else if(TASK_CMD_SRC_SELF == taskCommandPtr->cmdSrc)
            {
                ...
                //自身指令处理
                TaskFuncServiceParam_SelfMsg_Proc(selfCmdPtr);
                //测试完成,释放自身申请的内存
...
static void TaskServiceParam_Auto_Init(void)
{
    ServiceImplParam_StateMachineInit();
}

//系统循环自动程序
static void TaskServiceParam_Auto_Proc(void)
{
    ServiceImplParam_StateMachineLoop();
}

/************************************************CAN总线接收到的消息的处理******************************************************/
//读取参数
static LH_ERR ServiceTaskFuncParam_CanMsg_Proc_Read(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr);
//写入参数
static LH_ERR ServiceTaskFuncParam_CanMsg_Proc_Write(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr);

//配置参数,代表系统回调列表以及参数需求
/****前一个是处理函数***************************************参数的最小长度*****/
static const SYSTEM_TASK_CAN_MSG_PROC_UNIT serviceTaskCanMsgProcParamSetting[] = {
    {ServiceTaskFuncParam_CanMsg_Proc_Read                           ,           2           ,"Read"    },
    {ServiceTaskFuncParam_CanMsg_Proc_Write                          ,           6           ,"Write"   },
};
...
static void TaskFuncServiceParam_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr)
{
    ...
    //函数调用
    errorCode = serviceTaskCanMsgProcParamSetting[commandValue].funcPtr(canMsgCachePtr,
                    serviceTaskCanMsgProcParamSetting[commandValue].debugDetailMsg);
...
static LH_ERR ServiceTaskFuncParam_CanMsg_Proc_Read(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr)
{
    ...
    //读取序号和想要设置的状态
    SERVICE_PARAM_PARAM_MAIN mainIndex = (SERVICE_PARAM_PARAM_MAIN)(cachePtrLocal->recvCacheArray[4]);
    uint8_t subIndex = cachePtrLocal->recvCacheArray[5];
    float paramResult = 0;
    //读取数据
    errorCode = ServiceImplParam_Read(mainIndex,subIndex,&paramResult);
    if(errorCode == LH_ERR_NONE)
    {
        ...
        //将读取到的数据返回
        CanBaseWriteFloatDataToBuffer(dataBufferSend,0,paramResult);
        //返回数据包
        CanSlaveSendDataPackWithRecvCache(cachePtrLocal,CAN_BASE_DATA_TC_PARAM_READ,dataBufferSend,4);
        //发送完成,释放内存
...
static LH_ERR ServiceTaskFuncParam_CanMsg_Proc_Write(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr)
{
    ...
    uint8_t subIndex = cachePtrLocal->recvCacheArray[5];
    //需要写入的参数值
    float paramWrite = CanBaseReadDataConvertToFloat(cachePtrLocal->recvCacheArray,6);
    //写入数据
    errorCode = ServiceImplParam_Write(mainIndex,subIndex,paramWrite);
    return errorCode;
}
