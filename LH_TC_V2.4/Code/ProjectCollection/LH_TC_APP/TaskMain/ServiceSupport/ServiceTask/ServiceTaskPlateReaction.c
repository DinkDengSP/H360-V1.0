...
**Author: DengXiaoJun
**Date: 2020-01-16 20:01:02
**LastEditors: DengXiaoJun
**LastEditTime: 2020-05-03 15:40:32
**FilePath: \LH_TC_APP_V2.3\TaskMain\ServiceSupport\ServiceTask\ServiceTaskPlateReaction.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
#pragma pack(8)
//任务堆栈
CPU_STK stackBufferServiceTaskPlateReaction[STK_SIZE_SERVICE_TASK_PLATE_REACTION]@".ccram";
#pragma pack()

//任务控制块
OS_TCB tcbServiceTaskPlateReaction@".ccram";


//自动运行程序的执行周期,单位毫秒
#define SERVICE_TASK_PLATE_REACTION_AUTO_FUNC_RUNNING_PERIOD_MS            500
//系统循环自动程序初始化
...
//系统服务任务接收几个方面的消息
//1. 来自CAN1的消息
static void TaskFuncServicePlateReaction_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr);
//2. 来自自身的消息
...
void ServiceTaskFuncPlateReaction(void *p_arg)
{
    ...
    while(1)
    {
		...
		if(currentStateCheckTick - lastStateCheckTick >= SERVICE_TASK_PLATE_REACTION_AUTO_FUNC_RUNNING_PERIOD_MS)
		{
			//上次发送时间和本次发送时间差了SERVICE_TASK_PLATE_REACTION_AUTO_FUNC_RUNNING_PERIOD_MS个时间
        ...
        taskCommandPtr = OSTaskQPend((OS_TICK)SERVICE_TASK_PLATE_REACTION_AUTO_FUNC_RUNNING_PERIOD_MS,
        ...
        if ((taskCommandPtr != NULL)&&(err == OS_ERR_NONE))
		{
            ...
            if(TASK_CMD_SRC_CAN == taskCommandPtr->cmdSrc)
            {
                ...
                //对这个数据进行处理
                TaskFuncServicePlateReaction_CanMsg_Proc(canRecvDatPtr);
                //处理完成
            ...
            else if(TASK_CMD_SRC_SELF == taskCommandPtr->cmdSrc)
            {
                ...
                //自身指令处理
                TaskFuncServicePlateReaction_SelfMsg_Proc(selfCmdPtr);
                //测试完成,释放自身申请的内存
...
/************************************************CAN总线接收到的消息的处理******************************************************/
//读取当前反应盘温度状态
static LH_ERR ServiceTaskFuncPlateReaction_CanMsg_Proc_ReadState(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr);

//配置参数,代表系统回调列表以及参数需求
/****前一个是处理函数***************************************参数的最小长度*****/
static const SYSTEM_TASK_CAN_MSG_PROC_UNIT serviceTaskCanMsgProcPlateReactionSetting[] = {
    {ServiceTaskFuncPlateReaction_CanMsg_Proc_ReadState                          ,           0           ,"ReadState"},
};
#define TASK_CAN_MSG_PLATE_REACTION_COMMAND_COUNT  ((sizeof(serviceTaskCanMsgProcPlateReactionSetting))/(sizeof(serviceTaskCanMsgProcPlateReactionSetting[0])))
...
static void TaskFuncServicePlateReaction_CanMsg_Proc(CAN_SLAVE_CHANNEL_CACHE* canMsgCachePtr)
{
    ...
    if(commandValue >= TASK_CAN_MSG_PLATE_REACTION_COMMAND_COUNT)
    {
        //指令不支持
    ...
    //识别长度是否支持,前四个字节是指令码
    if((4 +serviceTaskCanMsgProcPlateReactionSetting[commandValue].paramDataMinLength) > canMsgCachePtr->currentLength)
    {
        //参数长度不够
    ...
    //函数调用
    errorCode = serviceTaskCanMsgProcPlateReactionSetting[commandValue].funcPtr(canMsgCachePtr,
                    serviceTaskCanMsgProcPlateReactionSetting[commandValue].debugDetailMsg);
...
static LH_ERR ServiceTaskFuncPlateReaction_CanMsg_Proc_ReadState(void* canMsgCachePtr,uint8_t* detailDebugMsgPtr)
{
    ...
    errorState = ServiceImplPlateReaction_ReadState(&sensorValue);
    ...
    //返回数据包
    CanSlaveSendDataPackWithRecvCache(cachePtrLocal,CAN_BASE_DATA_TC_CHANNEL_PLATE_REACTION,dataBufferSend,4);
    //发送完成,释放内存
