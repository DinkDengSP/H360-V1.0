...
//删除指定地址的MCU FLASH Erase
LH_ERR MCU_FlashErase(uint32_t WriteAddr,uint32_t NumToWrite)
{
    //flash状态
...
//指定地址区间的Flash编程
LH_ERR MCU_FlashProgram(uint32_t WriteAddr,uint32_t NumToWrite,uint32_t* bufferWrite)
{
    //打开HSI
    RCC_HSICmd(ENABLE);
    //延时10MS
    ...
    {
        //写入数据
        if(FLASH_ProgramWord(WriteAddr,*bufferWrite)!=FLASH_COMPLETE)
        { 
            //写入异常
            //上锁
            FLASH_Lock();
