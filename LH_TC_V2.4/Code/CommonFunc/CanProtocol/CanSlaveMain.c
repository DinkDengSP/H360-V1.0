...
//发送数据代码,发送长包过程包
static void CanSlaveSendDataLongPackProcedureWithRecvCache(CAN_SLAVE_CHANNEL_CACHE* cachePtr,CAN_BASE_DATA_FLAG dataFlag,uint8_t* bufferPtr,uint16_t bufferLength,CanTxMsg* txMsg)
{
    //创建ID
...
//发送数据代码,短包一次性发送
static void CanSlaveSendDataShortPackWithRecvCache(CAN_SLAVE_CHANNEL_CACHE* cachePtr,CAN_BASE_DATA_FLAG dataFlag,uint8_t* bufferPtr,uint16_t bufferLength)
{
    //首先申请内存
    OS_ERR err;
    //首先申请内存
...
//发送数据长包代码
void CanSlaveSendDataLongPackWithRecvCache(CAN_SLAVE_CHANNEL_CACHE* cachePtr,CAN_BASE_DATA_FLAG dataFlag,uint8_t* bufferPtr,uint16_t bufferLength)
{
    OS_ERR err;
    //首先申请内存
    CanTxMsg* txMsg = NULL;
...
//发送数据代码
void CanSlaveSendDataPackWithRecvCache(CAN_SLAVE_CHANNEL_CACHE* cachePtr,CAN_BASE_DATA_FLAG dataFlag,uint8_t* bufferPtr,uint16_t bufferLength)
{
    if(bufferLength > 6)
    {
        //长包
