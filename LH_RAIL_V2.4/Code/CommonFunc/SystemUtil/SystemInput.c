...
LH_ERR SystemInputRead(SYSTEM_INPUT_INDEX inputIndex,BitAction* inputValue)
{
    ...
    if(inputIndex > SYSTEM_INPUT_RAIL_BOARD_ARM_SEN32_PE4)
    {
        //超过系统上限
        return LH_ERR_BOARD_MAIN_COMM_SYSTEM_IN_RANGE;
    }
    if(inputIndex >= SYSTEM_INPUT_RAIL_BOARD_ARM_SEN1_PH10)
    {
        //主控板资源
        inputValueLocal = BoardInReadSingle((BOARD_IN_PIN)(inputIndex - SYSTEM_INPUT_RAIL_BOARD_ARM_SEN1_PH10));
    ...
    else
    {
        ...
        can2Input.inputPinIndex = (CAN2_SUB_IN_PIN_INDEX)(inputIndex + (uint32_t)CAN2_SUB_IN_PIN_BOARD8_CPLD_NOP1);
        //读取状态
