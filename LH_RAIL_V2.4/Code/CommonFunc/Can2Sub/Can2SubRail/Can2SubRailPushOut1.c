#include "Can2SubRailPushOut1.h"

//轨道出样1指令序号
typedef enum CMD_RAIL_PUSHOUT1
{
    CMD_RAIL_PUSHOUT1_RESET                                 = 0X00000000,
    CMD_RAIL_PUSHOUT1_PUSH_ONCE_WITH_RETURN_ZERO            = 0X00000001,
    CMD_RAIL_PUSHOUT1_PUSH_ONCE_NOT_RETURN_ZERO             = 0X00000002,
    CMD_RAIL_PUSHOUT1_RETURN_ZERO                           = 0X00000003,
    CMD_RAIL_PUSHOUT1_GET_INDEX                             = 0X00000004,
    CMD_RAIL_PUSHOUT1_BUTTON_PUSH_ONCE_NOT_RETURN_ZERO      = 0X00000005,
}CMD_RAIL_PUSHOUT1;
...
LH_ERR Can2SubRailPushOut1ResetWhileReturn(RAIL_RESET_CORRECT_FLAG resetFlag)
{
    ...
    //设置板号和通道号
        ...
        can2SendCmd.channelNo = CAN2_RAIL_CHANNEL_PUSHOUT1;
    //申请内存
    ...
    //指令码设定
        can2SendCmd.commandCode = CMD_RAIL_PUSHOUT1_RESET;
...
LH_ERR Can2SubRailPushOut1PushOnceWithReturnZeroWhileReturn(void)
{
    ...
    //指令码设定
        can2SendCmd.commandCode = CMD_RAIL_PUSHOUT1_PUSH_ONCE_WITH_RETURN_ZERO;
        can2SendCmd.timeoutMs = 60000;
    //数据发送
        errCode = Can2SendCommandWhileReturn(&can2SendCmd,&can2ReadData);
    //发送完成,释放发送内存
...
LH_ERR Can2SubRailPushOut1PushOnceNotReturnZeroWhileReturn(uint8_t* pushOutIndexPtr)
{
    ...
    //申请内存
        ...
        can2SendCmd.paramLength = 0;
    //指令码设定
        can2SendCmd.commandCode = CMD_RAIL_PUSHOUT1_PUSH_ONCE_NOT_RETURN_ZERO;
        can2SendCmd.timeoutMs = 60000;
    //数据发送
        errCode = Can2SendCommandWhileReturn(&can2SendCmd,&can2ReadData);
    //发送完成,释放发送内存
    ...
    //接收到的数据标识是否正确
        if(CAN2_SUB_DATA_RAIL_PUSHOUT1_INDEX != Can2ProcUtilConvertArrayToUint16(can2ReadData.recvDataPtr,0))
        {
            //数据标志位错误
            UserMemFree(MEM_AXI_SRAM,can2ReadData.recvDataPtr);
...
LH_ERR Can2SubRailPushOut1ButtonPushOnceNotReturnZeroWhileReturn(uint8_t* pushOutIndexPtr)
{
    ...
    //申请内存
        ...
        can2SendCmd.paramLength = 0;
    //指令码设定
        can2SendCmd.commandCode = CMD_RAIL_PUSHOUT1_BUTTON_PUSH_ONCE_NOT_RETURN_ZERO;
        can2SendCmd.timeoutMs = 60000;
    //数据发送
        errCode = Can2SendCommandWhileReturn(&can2SendCmd,&can2ReadData);
    //发送完成,释放发送内存
...
LH_ERR Can2SubRailPushOut1ReturnZeroWhileReturn(void)
{
    ...
    //申请内存
        ...
        can2SendCmd.paramLength = 0;
    //指令码设定
        can2SendCmd.commandCode = CMD_RAIL_PUSHOUT1_RETURN_ZERO;
        can2SendCmd.timeoutMs = 60000;
    //数据发送
        errCode = Can2SendCommandWhileReturn(&can2SendCmd,&can2ReadData);
    //发送完成,释放发送内存
...
LH_ERR Can2SubRailPushOut1GetIndexWhileReturn(uint8_t* pushOutIndexPtr)
{
    ...
    //申请内存
        ...
        can2SendCmd.paramLength = 0;
    //指令码设定
        can2SendCmd.commandCode = CMD_RAIL_PUSHOUT1_GET_INDEX;
        can2SendCmd.timeoutMs = 60000;
    //数据发送
        errCode = Can2SendCommandWhileReturn(&can2SendCmd,&can2ReadData);
    //发送完成,释放发送内存
