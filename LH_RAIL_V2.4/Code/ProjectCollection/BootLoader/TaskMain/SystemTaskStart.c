...
**LastEditTime: 2020-07-02 16:57:20
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
void BoardDeviceInit(void)
{
    ...
    //还剩下一个串口,目前还不知道做啥用,RS4 UART6
        BoardRS_Init(BOARD_RS4_UART6, 115200, MCU_UART_LENGTH_8B, MCU_UART_STOPBIT1, MCU_UART_CHECK_MODE_NONE,
                    MCU_UART_HARD_CONTROL_NONE, NULL);
    //获取打印模块号
        ActionPrintf("Rail Bootloader,ModuleNO : %d\r\n",CAN1_SELF_MODULE_NO);
    //SRAM初始化
        do
        {
            ...
            ActionPrintf("BoardSRAM_Init ErrorCode : 0X%08X\r\n",deviceResult);
    ...
    //铁电存储器初始化
        do
        {
            ...
            ActionPrintf("BoardMB85RS2MT_Init ErrorCode : 0X%08X\r\n",deviceResult);
            BoardLedToogle(BOARD_LED_RED);
            CoreDelayMs(100);
        }while(1);
    //初始化完成,全部关闭
    ...
    //IAP单元初始化,没有
        ...
        if(iapState == BOARD_IAP_JUMP_TO_BOOT_CASE_UPDATE)
        {
            ...
            ActionPrintf("Rail Send PrepareOK\r\n");
