...
**Author: DengXiaoJun
**Date: 2020-02-19 16:30:57
**LastEditors: DengXiaoJun
**LastEditTime: 2020-02-28 13:36:03
**FilePath: \App\TaskMain\ServiceSupport\ServiceIpc\ServiceIpcRailPushIn1.c
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
{
    IPC_CMD_PUSH_IN1_RESET               = 0X0000,//进样1复位
    IPC_CMD_PUSH_IN1_PUSH_ONCE           = 0X0001,//进样1推进一次
    IPC_CMD_PUSH_IN1_PUSH_MULTI          = 0X0002,//进样1推进多次
    IPC_CMD_PUSH_IN1_GET_SENSOR_STATE    = 0X0003,//进样1获取传感器状态
}IPC_CMD_PUSH_IN1;

//IPC调用的数据结构
static IPC_DATA_RAIL_PUSH_IN1 dataIPC_RailPushIn1;
//互斥信号量,用于API的互斥访问
static OS_MUTEX mutexIPC_RailPushIn1;

void IPC_RailPushIn1DataInit(void)
{
    OS_ERR err;
    //创建互斥信号量
    OSMutexCreate((OS_MUTEX*	)&mutexIPC_RailPushIn1,
				  (CPU_CHAR*	)"mutexIPC_RailPushIn1",
                  (OS_ERR*		)&err);	
    //状态赋值
    dataIPC_RailPushIn1.dataMutexPtr = &mutexIPC_RailPushIn1;
    dataIPC_RailPushIn1.state = STATE_IPC_IDLE;
    dataIPC_RailPushIn1.resultValue = LH_ERR_NONE;
    dataIPC_RailPushIn1.can1ErrorCode = CAN1_REPORT_OK;
    dataIPC_RailPushIn1.can1ReportErrorLevel = CAN1_REPORT_ERR_LEVEL_NORMAL;
}
...
STATE_IPC IPC_RailPushIn1GetLastCommandStateAndResult(LH_ERR* resultCode,CAN1_REPORT_ERR_CODE* can1ErrorPtr,CAN1_REPORT_ERR_LEVEL* reportLevelResultPtr)
{
    OS_ERR err;
    //申请信号量
    OSMutexPend (&mutexIPC_RailPushIn1,0,OS_OPT_PEND_NON_BLOCKING,0,&err);
    if(err != OS_ERR_NONE)
    {
    ...
    //申请到了,说明执行结束
    //读取结果代码
    *resultCode = dataIPC_RailPushIn1.resultValue;
    //读取中位机错误代码
    *can1ErrorPtr = dataIPC_RailPushIn1.can1ErrorCode;
    //读取中位机错误等级
    *reportLevelResultPtr = dataIPC_RailPushIn1.can1ReportErrorLevel;
    //释放信号量
    OSMutexPost(&mutexIPC_RailPushIn1,OS_OPT_POST_NONE,&err);
    //返回动作完成
    return dataIPC_RailPushIn1.state;
}
...
LH_ERR IPC_RailPushIn1WaitLastReturn(CAN1_REPORT_ERR_CODE* can1ErrorPtr,CAN1_REPORT_ERR_LEVEL* reportLevelResultPtr)
{
    ...
    do
    {
        ...
        ipcState = IPC_RailPushIn1GetLastCommandStateAndResult(&errorCode,can1ErrorPtr,reportLevelResultPtr);
    ...
    //取消按键生效
    if(key1Valid == RAIL_SENSOR_VALID)
    {
...
/****************************************************发出指令不等待完成*******************************************************/
//动作API发送任务给动作API
static LH_ERR IPC_RailPushIn1SendCommandWhileAck(uint16_t cmdIndex,int32_t param1,int32_t param2,int32_t param3,int32_t param4,int32_t param5)
{
    OS_ERR err;
    //系统消息指针
    ...
    dataIPC_RailPushIn1.state = STATE_IPC_WAIT_ACK;
    //指令结果值设置为初始化状态
    dataIPC_RailPushIn1.resultValue = LH_ERR_NONE;
    dataIPC_RailPushIn1.can1ErrorCode = CAN1_REPORT_OK;
    dataIPC_RailPushIn1.can1ReportErrorLevel = CAN1_REPORT_ERR_LEVEL_NORMAL;
    ...
    //设置不同信道特有的数据指针
    utilSelfCmdPtr->commandDataSpecialChannel = (void*)(&dataIPC_RailPushIn1);
    
    //将申请的指针传递给对应的任务信道做处理
    OSTaskQPost((OS_TCB *)&tcbTaskServiceRailPushIn1, //向协议处理线程发送消息
                        (void *)systemTaskCmd,
    ...
    while(dataIPC_RailPushIn1.state == STATE_IPC_WAIT_ACK)
    {
        CoreDelayMs(5);
    }
...
{
    return IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_RESET,(int32_t)correctFlag,0,0,0,0);
}

//进样1推进一次,不等待完成
LH_ERR IPC_RailPushIn1PushOnceWhileAck(void)
{
    return IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_PUSH_ONCE,0,0,0,0,0);
}

//进样1推进多次直到传感器触发,不等待完成
LH_ERR IPC_RailPushIn1PushMultiWhileSensorTrigWhileAck(uint16_t retryMax)
{
    return IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_PUSH_MULTI,(int32_t)retryMax,0,0,0,0);
}

//进样1获取传感器状态
LH_ERR IPC_RailPushIn1GetSensorStateWhileAck(void)
{
    return IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_GET_SENSOR_STATE,0,0,0,0,0);
}
...
LH_ERR IPC_RailPushIn1ResetWhileReturn(RAIL_RESET_CORRECT_FLAG correctFlag,CAN1_REPORT_ERR_CODE* can1ErrorPtr,CAN1_REPORT_ERR_LEVEL* reportLevelResultPtr)
{
    ...
    //首先发出指令,不等待返回看看是否能够发送出去
        errorCode = IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_RESET,(int32_t)correctFlag,0,0,0,0);
        if(errorCode != LH_ERR_NONE)
        {
            return errorCode;
        }
...
LH_ERR IPC_RailPushIn1PushOnceWhileReturn(CAN1_REPORT_ERR_CODE* can1ErrorPtr,CAN1_REPORT_ERR_LEVEL* reportLevelResultPtr)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //首先发出指令,不等待返回看看是否能够发送出去
        errorCode = IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_PUSH_ONCE,0,0,0,0,0);
        if(errorCode != LH_ERR_NONE)
        {
            return errorCode;
        }
...
LH_ERR IPC_RailPushIn1PushMultiWhileSensorTrigWhileReturn(uint16_t retryMax,CAN1_REPORT_ERR_CODE* can1ErrorPtr,CAN1_REPORT_ERR_LEVEL* reportLevelResultPtr)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //首先发出指令,不等待返回看看是否能够发送出去
        errorCode = IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_PUSH_MULTI,(int32_t)retryMax,0,0,0,0);
        if(errorCode != LH_ERR_NONE)
        {
            return errorCode;
        }
...
LH_ERR IPC_RailPushIn1GetSensorStateWhileReturn(CAN1_REPORT_ERR_CODE* can1ErrorPtr,CAN1_REPORT_ERR_LEVEL* reportLevelResultPtr)
{
    LH_ERR errorCode = LH_ERR_NONE;
    //首先发出指令,不等待返回看看是否能够发送出去
        errorCode = IPC_RailPushIn1SendCommandWhileAck(IPC_CMD_PUSH_IN1_GET_SENSOR_STATE,0,0,0,0,0);
        if(errorCode != LH_ERR_NONE)
        {
            return errorCode;
        }
