...
** Date          : 2022-11-14 13:52:08 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-11-14 14:03:47 +0800
************************************************************************************************/ 
#include "MCU_SPI2.h"
#include "MCU_Port.h"
#include "tx_mutex.h"
...
//保存配置
static SPI123_CONFIG spi2Config;
//互斥量
TX_MUTEX mutexMCU_SPI2;
//SPI2操作句柄
...
{
    //IO口复位
    MCU_PortDeInit(MCU_SPI2_PIN_SCK);
    MCU_PortDeInit(MCU_SPI2_PIN_MISO);
    MCU_PortDeInit(MCU_SPI2_PIN_MOSI);
    //模块复位
...
static void MCU_SPI2_MspInit(void)
{
    ...
    __HAL_RCC_SPI2_CLK_DISABLE();
    //选择时钟端口
    RCC_PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_SPI2;
    ...
    //IO口初始化
    //SCK
    MCU_PortInit(MCU_SPI2_PIN_SCK,MCU_PORT_MODE_AF_PP,MCU_PORT_PULL_PULLDOWN,MCU_PORT_SPEED_FREQ_HIGH,GPIO_AF5_SPI2);
    //MISO
    MCU_PortInit(MCU_SPI2_PIN_MISO,MCU_PORT_MODE_AF_PP,MCU_PORT_PULL_PULLDOWN,MCU_PORT_SPEED_FREQ_HIGH,GPIO_AF5_SPI2);
    //MOSI
    MCU_PortInit(MCU_SPI2_PIN_MOSI,MCU_PORT_MODE_AF_PP,MCU_PORT_PULL_PULLDOWN,MCU_PORT_SPEED_FREQ_HIGH,GPIO_AF5_SPI2);
}
...
//MCU SPI基础初始化
static void MCU_SPI2_BaseConfig(MCU_SPI_123_CLK speed,MCU_SPI_CPOL cpol,MCU_SPI_CPHA cpha)
{
	...
	SPI2_Handler.Init.CRCLength         = SPI_CRC_LENGTH_8BIT;
...
{
    if(TX_MUTEX_ID == mutexMCU_SPI2.tx_mutex_id)
    {
        return;
    }
    //创建互斥信号量
...
LH_ERR MCU_SPI2_ReadWriteBuffer(uint8_t* writeDataPtr,uint8_t* readDataPtr,uint16_t dataLength)
{
    ...
    if(halState != HAL_OK)
    {
        ...
        //显示错误
        LowLevelShowHalError(halState);
        return LH_ERR_MCU_SPI2_TRANS_TIME_OUT;
    }
    //释放锁
    MCU_SPI2_ReleaseLock();
    return LH_ERR_NONE;
...
//等待传输取消完成标记
static LH_ERR MCU_SPI2_WaitAbortTransComplete(uint32_t timeOutMs)
{
    ...
    return LH_ERR_MCU_SPI2_WAIT_ABORT_COMPLETE;
}
...
void MCU_SPI2_Init(MCU_SPI_123_CLK speed,MCU_SPI_CPOL cpol,MCU_SPI_CPHA cpha)
{
    ...
    //配置完成,配置相关的中断
    HAL_NVIC_SetPriority(SPI2_IRQn, SPI2_PREEMPT_PRIORITY_INT, SPI2_SUB_PRIORITY_INT);
...
//SPI取消传输
LH_ERR MCU_SPI2_AbortTrans(uint32_t timeOutMs)
{
...
LH_ERR MCU_SPI2_ReadWriteBuffer(uint8_t* writeDataPtr,uint8_t* readDataPtr,uint16_t dataLength)
{
    ...
    if(halState != HAL_OK)
    {
        ...
        //错误日志打印
        LowLevelShowHalError(halState);
        return LH_ERR_MCU_SPI2_TRANS_IT_START;
    }
    //开始等待时间
    ...
    do
    {
        ...
        {
#if(MCU_SPI2_INT_ENABLE_SCHEDULE == 1)
            CoreDelayMinTick();
#else
            asm("nop");
#endif
    ...
    if(transState == SPI_TRANS_STATE_START)
    {
        ...
        MCU_SPI2_ReleaseLock();
        //错误日志打印
        LowLevelShowErrorSubCode(LH_ERR_MCU_SPI2_TRANS_TIME_OUT);
        return LH_ERR_MCU_SPI2_TRANS_TIME_OUT;
    }
    ...
    else if(transState == SPI_TRANS_STATE_ERROR)
    {
        ...
        MCU_SPI2_ReleaseLock();
        //错误日志打印
        LowLevelShowErrorSubCode(LH_ERR_MCU_SPI2_TRANS_IT_ERR);
        return LH_ERR_MCU_SPI2_TRANS_IT_ERR;
    }
    //莫名其妙的错误,取消传输
    MCU_SPI2_AbortTrans(MCU_SPI2_ABORT_WAIT_TIME_MAX);
    //居然是莫名其妙的其他状态
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_AbortCpltCallback_IT\r\n");
#endif
}
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_TxRxCpltCallback_Int\r\n");
#endif
}
...
void MCU_SPI2_TxCpltCallback_Int(void)
{
    ...
    transState = SPI_TRANS_STATE_END;
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_TxCpltCallback_Int\r\n");
#endif
}
...
void MCU_SPI2_RxCpltCallback_Int(void)
{
    ...
    transState = SPI_TRANS_STATE_END;
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_RxCpltCallback_Int\r\n");
#endif
}
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_ErrorCallback_Int\r\n");
#endif
}
...
//表示指令已发送
#define EVENT_FLAG_SPI2_TX_COMPLETE             (1<<0)
//表示指令已接收
#define EVENT_FLAG_SPI2_RX_COMPLETE             (1<<1)
//表示指令收发完成
#define EVENT_FLAG_SPI2_TXRX_COMPLETE           (1<<2)
//表示指令已报错
#define EVENT_FLAG_SPI2_ERROR_COMPLETE          (1<<3)
//表示指令已终止
#define EVENT_FLAG_SPI2_ABORT_COMPLETE          (1<<4)
//需要与DMA共享的内存都设置成D2_SRAM1_DMA
#pragma pack(push) //保存对齐状态 
#pragma pack(4)
//用于DMA发送数据快
static uint8_t bufferWithSpi2Send[LENGTH_BYTES_SPI2_DMA_BUF]@".d2_sram1_dma" = {0};
//用于DMA接收数据块
static uint8_t bufferWithSpi2Recv[LENGTH_BYTES_SPI2_DMA_BUF]@".d2_sram1_dma" = {0};
#pragma pack (pop)
...
static void MCU_SPI2_DMA_Init(void)
{
    ...
    SPI2_DMA_TX_Handler.Init.Request             = DMA_REQUEST_SPI2_TX;
    //内存到外设
    ...
    SPI2_DMA_TX_Handler.Init.MemBurst            = DMA_MBURST_INC4;	// DMA_MBURST_INC4; DMA_MBURST_SINGLE
    SPI2_DMA_TX_Handler.Init.PeriphBurst         = DMA_MBURST_INC4;	// DMA_PBURST_INC4; DMA_PBURST_SINGLE
    ...
    //发送DMA中断设定
    HAL_NVIC_SetPriority(SPI2_DMA_TX_IRQ, SPI2_DMA_TX_PREEMPT_PRIORITY, SPI2_DMA_TX_SUB_PRIORITY);
    ...
    SPI2_DMA_RX_Handler.Init.Request             = DMA_REQUEST_SPI2_RX;
    //外设到内存
    ...
    SPI2_DMA_RX_Handler.Init.MemBurst            = DMA_PBURST_INC4;	// DMA_MBURST_INC4;  DMA_MBURST_SINGLE
    SPI2_DMA_RX_Handler.Init.PeriphBurst         = DMA_PBURST_INC4;	// DMA_PBURST_INC4;  DMA_PBURST_SINGLE
    ...
    //接收DMA中断设定
    HAL_NVIC_SetPriority(SPI2_DMA_RX_IRQ, SPI2_DMA_RX_PREEMPT_PRIORITY, SPI2_DMA_RX_SUB_PRIORITY);
...
void MCU_SPI2_Init(MCU_SPI_123_CLK speed,MCU_SPI_CPOL cpol,MCU_SPI_CPHA cpha)
{
    ...
    MCU_SPI2_BaseConfig(speed,cpol,cpha);
    //配置完成,配置相关的中断
    HAL_NVIC_SetPriority(SPI2_IRQn, SPI2_PREEMPT_PRIORITY_DMA, SPI2_SUB_PRIORITY_DMA);
	HAL_NVIC_EnableIRQ(SPI2_IRQn);
    //DMA初始化
    MCU_SPI2_DMA_Init();
}
...
LH_ERR MCU_SPI2_ReadWriteBuffer(uint8_t* writeDataPtr,uint8_t* readDataPtr,uint16_t dataLength)
{
    ...
    {
        UserMemSet(bufferWithSpi2Send,0,LENGTH_BYTES_SPI2_DMA_BUF);
    }
    //获取锁
    MCU_SPI2_GetLock();
    for(indexUtil = 0; indexUtil < sendPackCountMax;indexUtil++)
    {
        ...
        {
            //发送数据拷贝
            UserMemCopy(bufferWithSpi2Send,writeDataPtr+packPtrOffset,singlePackLength);
        }
        if(readDataPtr != SPI_PTR_NULL)
        {
            //接收数据清空
            UserMemSet(bufferWithSpi2Recv,0,LENGTH_BYTES_SPI2_DMA_BUF);
        }
        //清空信号量标记
        ...
        //DMA数据发送,同时收发
        halState = HAL_SPI_TransmitReceive_DMA(&SPI2_Handler, (uint8_t*)bufferWithSpi2Send, (uint8_t *)bufferWithSpi2Recv, 
                                                    singlePackLength);
        if(halState != HAL_OK)
        {
            //释放锁
            MCU_SPI2_ReleaseLock();
            //错误日志打印
            LowLevelShowErrorSubCode(LH_ERR_MCU_SPI2_TRANS_DMA_START);
            return LH_ERR_MCU_SPI2_TRANS_DMA_START;
        }
        //开始等待
        ...
        if(eventFlagsActualFlag&EVENT_FLAG_SPI2_ERROR_COMPLETE)
        {
            ...
            MCU_SPI2_ReleaseLock();
            //错误日志打印
            LowLevelShowErrorSubCode(LH_ERR_MCU_SPI2_TRANS_DMA_ERR);
            return LH_ERR_MCU_SPI2_TRANS_DMA_ERR;
        }
        //传输成功,数据拷贝
        if(readDataPtr != SPI_PTR_NULL)
        {
            //如果不使用扩展缓冲区,就调用这个清cache
            //SCB_InvalidateDCache_by_Addr((uint32_t *)bufferWithSpi2Recv, LENGTH_BYTES_SPI2_DMA_BUF);
            //完成之后进行接收数据拷贝
            UserMemCopy(readDataPtr+packPtrOffset,bufferWithSpi2Recv,singlePackLength);
        }
    }
    //释放锁
    MCU_SPI2_ReleaseLock();
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_AbortCpltCallback_DMA\r\n");
#endif
}
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_TxRxCpltCallback_DMA\r\n");
#endif
}
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_TxCpltCallback_DMA\r\n");
#endif
}
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_RxCpltCallback_DMA\r\n");
#endif
}
...
#if(MCU_SPI2_ENABLE_INT_SHOW == 1)
    LowLevelSendStringInt("MCU_SPI2_ErrorCallback_DMA\r\n");
#endif
}
