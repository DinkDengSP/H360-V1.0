...
**LastEditTime: 2021-08-09 20:10:48
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
    ...
    else
    {
        ...
        do
        {
            ...
            if (netCmdRecvStatePtr->recvDataLength < TCP_CMD_HEADER_SIZE)
            {
                ...
                //拷贝指定长度的数据量
                UserMemCopy((void*)(netCmdRecvStatePtr->recvDat+netCmdRecvStatePtr->recvDataLength), recvDataPtr+recvDataProcessIndex, copySizeUtil);
                //已经拷贝的数据量增加
...
LH_ERR SrvImplNetRecvWithCompletePack(NET_CMD_RECV_STATE* recvCompletePack)
{
    ...
    //申请内存,存放指令包
    netCommandPackPtr = UserMemMalloc(MEM_SDRAM1,(sizeof(NET_COMMAND_PACK)/sizeof(uint8_t)));
    ...
    if ((uint16_t)checkSumUtil != netCommandPackPtr->headerDataCheckSum)
    {
        ...
        //释放内存
        UserMemFree(MEM_SDRAM1,netCommandPackPtr);
        //回传NACK,让上位机重发
    ...
    {
        netCommandPackPtr->bodyData = UserMemMalloc(MEM_SDRAM1,netCommandPackPtr->bodyDataLength);
    ...
    if(errorCode != LH_ERR_NONE)
    {
        ...
        if(netCommandPackPtr->bodyDataLength != 0)
        {
            UserMemFree(MEM_SDRAM1,netCommandPackPtr->bodyData);
        }
        UserMemFree(MEM_SDRAM1,netCommandPackPtr);
    }
...
LH_ERR  SrvImplNetRecvDispatchCommand(NET_COMMAND_PACK* commandPackPtr)
{
    ...
    if(commandPackPtr->netCommand == TCP_CMD_COMMUNICATE_ACK)
    {
        ...
        {
            //释放数据包
            UserMemFree(MEM_SDRAM1,commandPackPtr->bodyData);
        }
        UserMemFree(MEM_SDRAM1,commandPackPtr);
        return LH_ERR_NONE;
    }
