...
** Date          : 2022-11-24 18:47:33 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-11-24 21:11:40 +0800
************************************************************************************************/ 
...
//NETX 驱动层保存的协议栈讯息
static NETX_PORT_INFO nxPortInfo;
//内存池控制块
...
#pragma pack(4)
/* ARP缓存 */
ULONG lan8700ArpTableBufNetX[NX_LAN8700_ARP_LENGTH/sizeof(ULONG)]@".dtcm_ram"; 
#pragma pack (pop)   
...
NX_PACKET_POOL* NetX_DriverLAN8700_CreateMemoryPool(void)
{
    ...
    //创建内存池
    if(nxPacketPoolLan8700.nx_packet_pool_id == NX_PACKET_POOL_ID)
    {
        //已经创建好,不需要重复创建
...
static uint8_t NetX_DriverLAN8700_RequestCheck(NX_IP_DRIVER *nxLan8700ReqPtr)
{
    ...
    if(nxPortInfo.nxPortInfoIP_Ptr != nxLan8700ReqPtr->nx_ip_driver_ptr)
    {
        ...
        //错误打印
        LowLevelPrintf("%s MultiCastJoin IP Ptr NotMatch\r\n",__func__);
        //失败计数增加
        nxPortInfo.errorCount++;
        return 1;
    }
    //接口匹配
    if(nxPortInfo.nxPortInfoInterfacePtr != nxLan8700ReqPtr->nx_ip_driver_interface)
    {
        ...
        nxLan8700ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL;
        //错误打印
        LowLevelPrintf("%s MultiCastJoin Interface Ptr NotMatch\r\n",__func__);
        //失败计数增加
        nxPortInfo.errorCount++;
        return 1;
    }
    ...
    if(nxPortInfo.nxPortInfoIP_Ptr != &lan8700NetX)
    {
        ...
        nxLan8700ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL;
        //错误打印
        LowLevelPrintf("%s MultiCastJoin IP Ptr NotLocal\r\n",__func__);
        //失败计数增加
        nxPortInfo.errorCount++;
        return 1;
    }
...
static void NetX_DriverLAN8700_Initialize(NX_IP_DRIVER *nxLan8700ReqPtr)
{
    ...
    //硬件初始化
    errorSubCode = BoardLAN8700_Init();
    if(errorSubCode != ERROR_SUB_OK)
    {
        LowLevelPrintf("BoardLAN8700_Init Failed : 0X%08X\r\n",errorSubCode);
        //状态切换为已经接口附着
    ...
    //检测网线连接状态
    errorSubCode = BoardLAN8700_GetLinkState(&(nxPortInfo.netPortState));
    if(errorSubCode != ERROR_SUB_OK)
    {
        //读取网口状态失败
        LowLevelPrintf("BoardLAN8700_GetLinkState Failed : 0X%08X\r\n",errorSubCode);
        //状态切换为已经接口附着
        nxPortInfo.nxPortInfoState = NETX_PORT_STATE_ATTACH;
        //回传失败
        nxLan8700ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL;
...
uint8_t NetX_DriverLAN8700_PacketSendCheckLinkState(NX_IP_DRIVER *nxLan8700ReqPtr)
{
    ...
    //解析速度和双工模式
    BoardLAN8700_GetModeSpeed(&(nxPortInfo.linkSpeed),&(nxPortInfo.deplexMode));
    //根据网络接口状态修正MAC层
    MCU_ETH_ReflushModeSpeed(nxPortInfo.netPortState);
    //决定是否可以发送
    if((nxPortInfo.netPortState == NET_PORT_STATE_LINK_DOWN)||(nxPortInfo.netPortState == NET_PORT_STATE_AUTONEGO_NOTDONE)||
...
static void NetX_DriverLAN8700_PacketSend(NX_IP_DRIVER *nxLan8700ReqPtr)
{
    ...
    *(ethernet_frame_ptr + 3) = (ip_ptr->nx_ip_arp_physical_address_lsw << 16);
    ...
    nxLan8700ReqPtr->nx_ip_driver_status = NX_SUCCESS;
    //发送计数
    nxPortInfo.sendPackCount += indexUtil;
    return;
}
