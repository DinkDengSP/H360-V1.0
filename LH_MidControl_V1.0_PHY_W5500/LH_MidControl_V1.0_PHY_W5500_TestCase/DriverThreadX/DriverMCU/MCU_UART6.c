...
** Date          : 2022-11-24 18:02:13 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-11-24 20:09:25 +0800
************************************************************************************************/ 
#include "MCU_UART6.h"
#include "MCU_PIN.h"
#include "MCU_RTT.h"
...
#pragma pack(4)
//用于当前发送的数据块
static uint8_t blockBufferWithUart6SendCurrent[LENGTH_UART6_BUFFER_WITH_BLOCK]@".d2_sram2_dma" = {0};
//用于循环队列存储数据的数组
static uint8_t bufferWithUart6SendQueue[LENGTH_UART6_BUFFER_WITH_SEND]@".d2_sram2_dma" = {0};
#pragma pack (pop)
#endif
...
//接收缓存
static UART_RECV_PACK uart6RecvPackArray[COUNT_UART6_RECV_BLOCK];
//接收缓存
...
//当前接收对象
static uint8_t currentBufferWithUart6Recv[LENGTH_UART6_RECV_SINGLE_BLOCK]@".d2_sram2_dma" = {0};
#endif  
...
static void MCU_UART6_MspInit(void)
{
    ...
    MCU_PinInit(UART6_PIN_TX,MCU_PIN_MODE_AF_PP,MCU_PIN_PULL_PULLUP,MCU_PIN_SPEED_FREQ_HIGH,GPIO_AF7_USART6);
#endif
#if((MCU_UART6_TRANS_MODE == UART_MODE_TX_RX)||(MCU_UART6_TRANS_MODE == UART_MODE_RX))
    MCU_PinInit(UART6_PIN_RX,MCU_PIN_MODE_AF_PP,MCU_PIN_PULL_PULLUP,MCU_PIN_SPEED_FREQ_HIGH,GPIO_AF7_USART6);
#endif
}
...
{
#if((MCU_UART6_TRANS_MODE == UART_MODE_TX_RX)||(MCU_UART6_TRANS_MODE == UART_MODE_TX))
    MCU_PinDeInit(UART6_PIN_TX);
#endif
#if((MCU_UART6_TRANS_MODE == UART_MODE_TX_RX)||(MCU_UART6_TRANS_MODE == UART_MODE_RX))
    MCU_PinDeInit(UART6_PIN_RX);
#endif
    //关闭中断
                        ...
                        MCU_UartRecvIntProcFunc rxCallBack)
{
    ...
    UART6_Handler.Init.OverSampling = MCU_UART6_OVERSAMPLING_MODE;
    //采样值的位数,是否打开一个采样位方法,打开的话,将禁止噪声检测标记
    UART6_Handler.Init.OneBitSampling = MCU_UART6_ONE_BIT_SAMPLE;
    //关闭FIFO模式
    UART6_Handler.FifoMode = UART_FIFOMODE_DISABLE;
    //高级配置,其中可以设置端口反转和电平反转,根据需要设计自己的高级特性相或
    uint32_t advFeatureInit = UART_ADVFEATURE_NO_INIT;
    //是否关闭ORE过载标记
#if(MCU_UART6_DISABLE_ORE != 0)
    advFeatureInit |= UART_ADVFEATURE_RXOVERRUNDISABLE_INIT;
#endif
    UART6_Handler.AdvancedInit.AdvFeatureInit = advFeatureInit;
//是否打开接收
#if((MCU_UART6_TRANS_MODE == UART_MODE_TX_RX)||(MCU_UART6_TRANS_MODE == UART_MODE_RX))
    //设置中断缓冲
...
void MCU_UART6_RecvDataInit(void)
{
    ...
    {
        uart6RecvPackArray[indexUtil].packSrc = UART_PACK_SRC_UART6;
        uart6RecvPackArray[indexUtil].recvDataBufferPtr = (uint8_t*)(blockBufferWithUart6Recv[indexUtil]);
        uart6RecvPackArray[indexUtil].recvDataLength = 0;
    }
    //队列导入
    for(indexUtil = 0;indexUtil < COUNT_UART6_RECV_BLOCK;indexUtil++)
    {
        ADT_CircularPtrQueueWrite(&circularPtrQueueUart6RecvIdleBlock,(void*)(&(uart6RecvPackArray[indexUtil])));
    }
    //清除当前接收队列
...
static void MCU_UART6_StartRecv(void)
{
    ...
    HAL_UARTEx_ReceiveToIdle_IT(&UART6_Handler, (uint8_t *)(currentBufferWithUart6Recv), LENGTH_UART6_RECV_SINGLE_BLOCK);
#endif
#if(MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA)
    //开始DMA接收中断
    HAL_UARTEx_ReceiveToIdle_DMA(&UART6_Handler,(uint8_t *)(currentBufferWithUart6Recv), LENGTH_UART6_RECV_SINGLE_BLOCK);
#endif
}
...
void MCU_UART6_RecvDatRelease(UART_RECV_PACK* uartRecvPackPtr)
{
    ...
    UserMemClear((void*)(uartRecvPackPtr->recvDataBufferPtr),LENGTH_UART6_RECV_SINGLE_BLOCK);
    //数据指针写回
    ADT_CircularPtrQueueWrite(&circularPtrQueueUart6RecvIdleBlock,uartRecvPackPtr);
#endif
}
...
void MCU_UART6_RxCpltCallBack(void)
{
    ...
    //从队列中拿出一个指针做接收转发
    UART_RECV_PACK* recvDispatchPtr = ADT_CircularPtrQueueGet(&circularPtrQueueUart6RecvIdleBlock);
    ...
    {
        //配置接收参数
        recvDispatchPtr->packSrc = UART_PACK_SRC_UART6;
        recvDispatchPtr->recvDataLength = LENGTH_UART6_RECV_SINGLE_BLOCK;
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->recvDataBufferPtr),(void*)currentBufferWithUart6Recv,LENGTH_UART6_RECV_SINGLE_BLOCK);
        //清空接收
        UserMemClear((void*)currentBufferWithUart6Recv,LENGTH_UART6_RECV_SINGLE_BLOCK);
#if(MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
        //该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量
...
#endif   
        //数据转发
        if(ERROR == uart6RecvFuncPtr(recvDispatchPtr))
        {
            //转发失败,将数据放回去
            MCU_UART6_RecvDatRelease(recvDispatchPtr);
        }
    }
#endif
}
...
{
    //关闭接收
    ATOMIC_CLEAR_BIT(UART6_Handler.Instance->CR1, USART_CR1_RE);
    //诊断中断来源并打印到RTT中
    if ((UART6_Handler.ErrorCode & HAL_UART_ERROR_ORE) != RESET)
    {
        //接收溢出中断
        __HAL_UART_CLEAR_OREFLAG(&UART6_Handler);   
        MCU_RTT_SendStringInt("MCU_UART6_ErrorCallback HAL_UART_ERROR_ORE\r\n");
    }
    if ((UART6_Handler.ErrorCode & HAL_UART_ERROR_FE) != RESET)
    {
        //帧错误中断
        __HAL_UART_CLEAR_FEFLAG(&UART6_Handler);
        //错误打印
        MCU_RTT_SendStringInt("MCU_UART6_ErrorCallback HAL_UART_ERROR_FE\r\n");
    }
    if ((UART6_Handler.ErrorCode & HAL_UART_ERROR_PE) != RESET)
    {
        //奇偶校验中断
        __HAL_UART_CLEAR_PEFLAG(&UART6_Handler);
        //错误打印
        MCU_RTT_SendStringInt("MCU_UART6_ErrorCallback HAL_UART_ERROR_PE\r\n");
    }
    if ((UART6_Handler.ErrorCode & HAL_UART_ERROR_NE) != RESET)
    {
        //奇偶校验中断
        __HAL_UART_CLEAR_NEFLAG(&UART6_Handler);
        //错误打印
        MCU_RTT_SendStringInt("MCU_UART6_ErrorCallback HAL_UART_ERROR_NE\r\n");
    }
    if ((UART6_Handler.ErrorCode & HAL_UART_ERROR_DMA) != RESET)
    {
        //错误打印
        MCU_RTT_SendStringInt("MCU_UART6_ErrorCallback HAL_UART_ERROR_DMA\r\n");
    }
    //重新启动接收
...
#endif
    //重新使能接收
    ATOMIC_SET_BIT(UART6_Handler.Instance->CR1, USART_CR1_RE);
}
...
{
#if(MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
    if((dataLength == 0)||(uart6RecvFuncPtr == NULL))
    {
        //清空接收
        UserMemClear((void*)currentBufferWithUart6Recv,LENGTH_UART6_RECV_SINGLE_BLOCK);
        //打开空闲接收
    ...
    else
    {
        ...
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->recvDataBufferPtr),(void*)currentBufferWithUart6Recv,dataLength);
        //清空接收
        UserMemClear((void*)currentBufferWithUart6Recv,LENGTH_UART6_RECV_SINGLE_BLOCK);
        //打开空闲接收
        HAL_UARTEx_ReceiveToIdle_IT(&UART6_Handler,(uint8_t *)(currentBufferWithUart6Recv), LENGTH_UART6_RECV_SINGLE_BLOCK);
...
#endif
#if(MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA) 
    if(UART6_Handler.RxState != HAL_UART_STATE_READY)
    {
