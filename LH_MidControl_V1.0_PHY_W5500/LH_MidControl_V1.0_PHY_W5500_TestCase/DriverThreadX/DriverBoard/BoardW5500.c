...
** Date          : 2022-11-24 18:10:29 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2022-11-25 10:20:48 +0800
************************************************************************************************/ 
...
//配置MAC地址
ERROR_SUB BoardW5500_ConfigMacAddrDefault(void)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
...
//配置子网掩码
ERROR_SUB BoardW5500_ConfigSubNetMaskDefault(void)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
...
//打开socket
static ERROR_SUB BoardW5500_OpenSocketMacRaw(void)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
...
//重新打开Socket
ERROR_SUB BoardW5500_ReopenSocketMacRaw(void)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //获取互斥信号量
    BoardW5500_GetLock();
...
//启动数据发送
static ERROR_SUB BoardW5500_StartSendMackRaw(void)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //辅助变量
    uint8_t utilTempByte = 0;
...
//等待发送完成
static ERROR_SUB BoardW5500_WaitSendOverMacRaw(uint32_t timeOutMs)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //辅助变量
    uint8_t utilTempByte = 0;
    ...
    do
    {
        ...
        if((utilTempByte&W5500_REG_VAL_SOCKET_Sn_IR_TIMEOUT) != 0)
        {
            ...
            BoardW5500_WriteSockeIntReg(W5500_SOCKET_NO_0,0XFF);
            //打印错误代码
            LowLevelShowErrorSubCode(ERROR_SUB_BOARD_W5500_SEND_IR_TIME_OUT);
            return ERROR_SUB_BOARD_W5500_SEND_IR_TIME_OUT;
        }
        if(tx_time_get() - startWaitTime > timeOutMs)
        {
            ...
            BoardW5500_WriteSockeIntReg(W5500_SOCKET_NO_0,0XFF);
            //打印错误代码
            LowLevelShowErrorSubCode(ERROR_SUB_BOARD_W5500_SEND_USER_TIME_OUT);
            return ERROR_SUB_BOARD_W5500_SEND_USER_TIME_OUT;
        }
        //都没触发,最小延时
        CoreDelayMinTick();
    }while(1);
...
//继续接收
static ERROR_SUB BoardW5500_ContinueRecvMacRaw(void)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //辅助变量
    uint8_t utilTempByte = 0;
...
ERROR_SUB BoardW5500_SendOneFrameMacRaw(uint8_t* dataBufferPtr,uint16_t dataLength)
{
    ...
    do
    {
        ...
        if(utilTempByte == W5500_REG_VAL_SOCKET_Sn_SR_SOCK_CLOSED)
        {
            ...
            BoardW5500_ReleaseLock();
            //打印错误代码
            LowLevelShowErrorSubCode(ERROR_SUB_BOARD_W5500_SEND_CHECK_CLOSED);
            return ERROR_SUB_BOARD_W5500_SEND_CHECK_CLOSED;
        }
        //检查长度
