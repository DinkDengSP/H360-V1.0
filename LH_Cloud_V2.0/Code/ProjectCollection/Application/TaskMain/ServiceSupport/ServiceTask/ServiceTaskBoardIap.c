...
**LastEditTime: 2020-12-15 16:14:19
**ModifyRecord1:    
**ModifyRecord2:    
**ModifyRecord3:    
**ModifyRecord4:    
...
void ServiceTaskFuncBoardIap(void *p_arg)
{
    ...
    //延时等待接收函数初始化成功
    CoreDelayMs(100);
    if(bootLoaderNeedResp == 0XFE)
    {
        bootLoaderNeedResp = 0;
        CoreDelayMs(100);
        Can1ProtocolSendProgramJumpWithSelf(CAN1_SELF_BOARD_ID,1,1);
    }
    while(1)
    {
...
void ServiceTaskFuncBoardIap_Can1Msg_Proc_ProgramJump(CAN1_CMD_RECV* can1CommandPtr,uint8_t* detailDebugMsgPtr)
{
    ...
    {
       //APP不支持下位机和其他板卡的跳转
        Can1ProtocolSendProgramJumpResultWithCommand(can1CommandPtr,CAN1_SELF_BOARD_ID,0,1); 
    }
}
