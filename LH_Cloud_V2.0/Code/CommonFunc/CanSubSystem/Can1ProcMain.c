...
static CAN1_RECV_CACHE can1RecvCache;
...
{
    //ID解析
    Can1RecvAnalsisID(rxMsg->ExtId);
    //然后对数据进行拷贝
    can1CurrentRecvDataLength = rxMsg->DLC;
    for(uint8_t i = 0; i < can1CurrentRecvDataLength; i++)
    {
        can1CurrentRecvDataArray[i] = rxMsg->Data[i];
    }
...
void Can1DispatchReceive(CAN1_DISPATCH_UNIT* unitPtr,uint16_t unitElementCount)
{
    ...
    if(can1RecvCache.cmdRecvState == CAN1_RECV_STATE_END)
    {
        ...
        do
        {
            systemCmdPtr = UserMemMalloc(SRAM_CCM,sizeof(SYSTEM_CMD)/sizeof(uint8_t));
        ...
        do
        {
            can1CommandRecvPtr = UserMemMalloc(SRAM_CCM,sizeof(CAN1_CMD_RECV)/sizeof(uint8_t));
        ...
        if(indexOfElement == unitElementCount)
        {
            ...
            UserMemFree(SRAM_CCM,systemCmdPtr);
        }
        else
        {
            //找到了,数据转发
...
static void Can1SendCommandPackShort(CAN1_CMD_SEND* can1CmdPtr)
{
    ...
    //扩展帧
    txMsg.IDE = CAN_Id_Extended;txMsg.RTR = CAN_RTR_Data;
...
void Can1SendCommandResponseMsg(uint16_t cmdValue,CAN1_REPORT_ERR_CODE midError,CAN1_REPORT_ERR_LEVEL level,LH_ERR systemError,uint16_t paramLength,uint8_t* paramPtr)
{
    ...
    do
    {
        reportBuffer = UserMemMalloc(SRAM_CCM,7+paramLength);
    }while(reportBuffer == NULL);
    //中位机错误代码
    //枚举类型是int类型,需要转换成uint类型再进行位操作,才能保证结果的正确性
    ...
    //报警级别
    reportBuffer[3] = (uint8_t)level;
    //系统报警码
    utilData = (uint32_t)(systemError);
    reportBuffer[4] = (uint8_t)(utilData&0x00FF);
    reportBuffer[5] = (uint8_t)((utilData>>8)&0XFF);
    reportBuffer[6] = (uint8_t)((utilData>>16)&0XFF);
    reportBuffer[7] = (uint8_t)((utilData>>24)&0XFF);
    //标准格式之外的数据
    if(paramLength != 0)
    {
        //添加参数讯息
        UserMemCopy((void*)(reportBuffer+7),(void*)paramPtr,paramLength);
    }
    //设置发送参数
    can1SendCommand.sendDataBuffer = reportBuffer;
    can1SendCommand.sendDataLength = 7+paramLength;
...
void Can1SendCommandResult(CAN1_CMD_RECV* can1CommandPtr,CAN1_REPORT_ERR_CODE reportCode,CAN1_REPORT_ERR_LEVEL reportLevel,LH_ERR errorParam,uint16_t paramLength,uint8_t* paramPtr)
{
    ...
    //中位机错误代码
    //枚举类型是int类型,需要转换成uint类型再进行位操作,才能保证结果的正确性
    uint32_t utilData = (uint32_t)(reportCode);
    //中位机报警码
    reportBuffer[0] = (uint8_t)(utilData&0x00FF);
    reportBuffer[1] = (uint8_t)((utilData>>8)&0XFF);
    //报警级别
    reportBuffer[2] = (uint8_t)reportLevel;
    //系统报警码
    utilData = (uint32_t)(errorParam);
    reportBuffer[3] = (uint8_t)(utilData&0x00FF);
    reportBuffer[4] = (uint8_t)((utilData>>8)&0XFF);
    reportBuffer[5] = (uint8_t)((utilData>>16)&0XFF);
    reportBuffer[6] = (uint8_t)((utilData>>24)&0XFF);
    ...
    can1SendCommand.sendDataBuffer = reportBuffer;
    can1SendCommand.sendDataLength = 7+paramLength;
    can1SendCommand.command = can1CommandPtr->command;
    //发送一帧
    Can1SendCommandPack(&can1SendCommand);
    //发送完成,释放内存
    UserMemFree(SRAM_CCM,reportBuffer);
...
void Can1SendCommandResultWithLength(CAN1_CMD_RECV* can1CommandPtr,CAN1_REPORT_ERR_CODE reportCode,CAN1_REPORT_ERR_LEVEL reportLevel,LH_ERR errorParam,uint16_t paramLength,uint8_t* paramPtr)
{
    ...
    do
    {
        reportBuffer = UserMemMalloc(SRAM_CCM,9+paramLength);
    }while(reportBuffer == NULL);
    //中位机错误代码
    //枚举类型是int类型,需要转换成uint类型再进行位操作,才能保证结果的正确性
    uint32_t utilData = (uint32_t)(reportCode);
    ...
    reportBuffer[6] = (uint8_t)((utilData>>24)&0XFF);
    //返回数据长度
    utilData = (uint32_t)(paramLength);
    reportBuffer[7] = (uint8_t)(utilData&0x00FF);
    reportBuffer[8] = (uint8_t)((utilData>>8)&0XFF);
    //标准格式之外的数据
    if(paramLength != 0)
    {
        //添加参数讯息
...
void Can1ProtocolSendAppVersionWithCommand(CAN1_CMD_RECV* can1CommandPtr,uint8_t boardID,uint8_t mainVersion,uint8_t subVersion,uint8_t debugVersion,uint32_t svnVersion)
{
    ...
    Can1SendCommandPackWithRecvCmd(can1CommandPtr,CAN1_CMD_TEST_FLOW_CMD_CELL_VERSION,8,(uint8_t*)(sendBuffer));
}
...
void Can1ProtocolSendBootVersionWithCommand(CAN1_CMD_RECV* can1CommandPtr,uint8_t boardID,uint8_t mainVersion,uint8_t subVersion,uint8_t debugVersion,uint32_t svnVersion)
{
    ...
    sendBuffer[6] = (uint8_t)((svnVersion>>16)&0XFF);
    sendBuffer[7] = (uint8_t)((svnVersion>>24)&0XFF);
    Can1SendCommandPackWithRecvCmd(can1CommandPtr,CAN1_CMD_TEST_FLOW_CMD_CELL_BOOT_VERSION,8,(uint8_t*)(sendBuffer));
}
...
void Can1ProtocolSendIapEndResultWithCommand(CAN1_CMD_RECV* can1CommandPtr,uint8_t boardID,uint8_t result)
{
	...
	sendBuffer[1] = (uint8_t)(boardID);//板号
	sendBuffer[2] = (uint8_t)(result);//结果
    Can1SendCommandPackWithRecvCmd(can1CommandPtr,CAN1_CMD_TEST_FLOW_CMD_CELL_UPDATA_END,3,(uint8_t*)(sendBuffer));
}
...
void Can1ProtocolSendProgramJumpResultWithCommand(CAN1_CMD_RECV* can1CommandPtr,uint8_t boardID,uint8_t result,uint8_t state)
{
	...
	sendBuffer[2] = (uint8_t)(result);//结果
    sendBuffer[3] = state;
    Can1SendCommandPackWithRecvCmd(can1CommandPtr,CAN1_CMD_TEST_FLOW_CMD_CELL_PROGRAM_JUMP,4,(uint8_t*)(sendBuffer));
}
...
void Can1ProtocolSendSoftwareUpdatePrepareWithSelf(uint8_t boardID,uint8_t state)
{
    ...
    can1CmdSendResult.command = CAN1_CMD_TEST_FLOW_CMD_CELL_UPDATA_PREPARE;
...
void Can1ProtocolSendProgramJumpWithSelf(uint8_t boardID,uint8_t result,uint8_t state)
{
    ...
    can1CmdSendResult.targetModuleNo = CAN1_COMM_MODULE_NO;
    can1CmdSendResult.targetBoardAddr = CAN1_COMM_BOARD_ID;
    can1CmdSendResult.command = CAN1_CMD_TEST_FLOW_CMD_CELL_PROGRAM_JUMP;
